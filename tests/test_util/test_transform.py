...
class metafunctionsTests(TestCase):
    ...
    def test_all(self):
        ...
        self.assertEqual(
            map(odd_number, range(5)), [False, True] * 2 + [False])
        # should short-circuit, i.e. not evaluate later cases after False
    ...
    def test_any(self):
        ...
        self.assertEqual(odd_vowel('a'), True)
        self.assertEqual(
            map(odd_vowel, 'abceu'), [True, False, True, True, True])
    ...
    def test_none(self):
        ...
        self.assertRaises(
            AssertionError, none([self.is_odd_letter, self.fail]),
            'b')
    ...
    def test_index(self):
        ...
        i = str_indexer(items)
        self.assertEqual(
            i, {'5': [num, let], '0': [zer], 'None': [non], '3': [y]})
        # test order correct in duplicates
        i = str_indexer(duplicates)
        self.assertEqual(
            i, {'5': [num, let, num, let, num, let], '0': [zer, zer, zer],
