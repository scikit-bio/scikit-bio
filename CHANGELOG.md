...
* `dirmult_lme` now fits its default random-intercept model for all features of a posterior draw at once, using the closed form of the REML profile likelihood, instead of running one numeric optimization per feature through statsmodels' `MixedLM.fit`. Estimates, standard errors, *p*-values and confidence intervals are computed from the same profile likelihood and the same observed information matrix that `MixedLM` uses. Supplying `re_formula`, `vc_formula`, `model_kwargs`, `fit_method` or `fit_kwargs` selects a model or optimizer with no closed form and falls back to the previous per-feature path. Reported values can differ slightly from previous versions on replicates where `MixedLM`'s iterative optimizer stopped short of the optimum; in those cases the new fit attains the higher log-likelihood.
...
* Fixed `rda` ignoring `sample_ids`, `feature_ids` and `constraint_ids`. The parameters now label unlabelled tables, override existing labels, and reject a mismatched ID count, matching `cca` ([#2571](https://github.com/scikit-bio/scikit-bio/issues/2571)). Thank @Arthur031221 for reporting.
* Fixed gzip readers inferring write mode from read/write file objects on Python 3.15 by explicitly opening in binary read mode ([#2577](https://github.com/scikit-bio/scikit-bio/pull/2577)). Thanks to Maximiliano Curia, Andreas Tille and @ChristaCode.
...
* Fixed `GrammaredSequence.to_definites` raising `TypeError: 'NoneType' object is not subscriptable` on any sequence type that defines no non-canonical characters, which includes `DNA` and `RNA`. The cached mask builder `_degen_nonca_hash` assigned to `_canonical_hash` instead of itself in that branch, so it always returned `None` and additionally overwrote the canonical-character mask with the degenerate-character mask ([#2562](https://github.com/scikit-bio/scikit-bio/pull/2562)).
...
* The FASTA reader now includes the offending record's ID when a sequence fails validation while reading (e.g., an invalid character in one record among many), instead of raising an error with no indication of which record was at fault ([#1470](https://github.com/scikit-bio/scikit-bio/issues/1470)).
