...
class PERMANOVATests(TestCase):
    ...
    def test_fixed_point_distance_matrix(self):
        ...
        expected = permanova(float_dm, self.grouping_equal, permutations=99, seed=42)
        observed = permanova(fixed_dm, self.grouping_equal, permutations=99, seed=42)
        self.assert_series_equal(observed, expected)

        expected = permanova(float_condensed, self.grouping_equal, permutations=99, seed=42)
        observed = permanova(fixed_condensed, self.grouping_equal, permutations=99, seed=42)
        self.assert_series_equal(observed, expected)
