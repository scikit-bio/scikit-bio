...
class MantelTestData(TestCase):
    def setUp(self):
        ...
        self.minz_dm_extra = DistanceMatrix(
            [[0, 0.5, 0.25, 3], [0.5, 0, 0.1, 24], [0.25, 0.1, 0, 5], [3, 24, 5, 0]],
            ["0", "1", "2", "bar"],
        )
...
class InternalMantelTests(MantelTestData):
    ...
    def test_perm_pearsonr3(self):
        ...
        x_data = np.asarray([[0.0, 1.0, 3.0], [1.0, 0.0, 2.0], [3.0, 2.0, 0.0]])

        perm_order = np.asarray([[2, 1, 0], [2, 0, 1], [0, 2, 1], [2, 0, 1]], dtype=int)
        xmean = 2.0
    ...
    def test_perm_pearsonr6(self):
        ...
        x_data = np.asarray(
            [
                [0.0, 0.62381864, 0.75001543, 0.58520119, 0.72902358, 0.65213559],
                [0.62381864, 0.0, 0.97488122, 0.6498224, 0.73720314, 0.62950732],
                [0.75001543, 0.97488122, 0.0, 0.68884542, 0.65747031, 0.72170752],
                [0.58520119, 0.6498224, 0.68884542, 0.0, 0.65885358, 0.66122362],
                [0.72902358, 0.73720314, 0.65747031, 0.65885358, 0.0, 0.71117341],
                [0.65213559, 0.62950732, 0.72170752, 0.66122362, 0.71117341, 0.0],
            ]
        )
    ...
    def test_pearsonr_full(self):
        ...
        perm_gen = (
            pearsonr(x.permute(condensed=True), y_flat)[0] for _ in range(num_perms)
        )
    ...
    def test_spearmanr_full(self):
        ...
        np.random.seed(0)
        orig_stat_fast, comp_stat, permuted_stats_fast = _mantel_stats_spearman(
            x, y, num_perms
        )
        ...
        perm_gen = (
            spearmanr(x.permute(condensed=True), y_flat)[0] for _ in range(num_perms)
        )
        permuted_stats = np.fromiter(perm_gen, float, count=num_perms)
...
class MantelTests(MantelTestData):
    ...
    def setUp(self):
        ...
        self.no_variation = [[0, 0.667, 0.667], [0.667, 0, 0.667], [0.667, 0.667, 0]]
        ...
        self.veg_dm_vegan = np.loadtxt(get_data_path("mantel_veg_dm_vegan.txt"))
        self.env_dm_vegan = np.loadtxt(get_data_path("mantel_env_dm_vegan.txt"))
    ...
    def test_zero_permutations(self):
        for alt in self.alternatives:
            for method, exp in (
                ...
                obs = mantel(
                    self.minx, self.miny, permutations=0, method=method, alternative=alt
                )
                ...
                obs = mantel(
                    self.miny, self.minx, permutations=0, method=method, alternative=alt
                )
                self.assertAlmostEqual(obs[0], exp)
                npt.assert_equal(obs[1], np.nan)
                self.assertEqual(obs[2], 3)
    ...
    def test_one_sided_less(self):
        ...
        for method in self.methods:
            obs = mantel(self.minx, self.minx, method=method, alternative="less")
    ...
    def test_no_variation_pearson(self):
        for alt in self.alternatives:
            ...
            obs = mantel(
                self.no_variation, self.no_variation, method="pearson", alternative=alt
            )
            npt.assert_equal(obs, (np.nan, np.nan, 3))
    ...
    def test_no_variation_spearman(self):
        ...
        for alt in self.alternatives:
            obs = mantel(
                self.miny, self.no_variation, method="spearman", alternative=alt
            )
            npt.assert_equal(obs, exp)

            obs = mantel(
                self.no_variation, self.miny, method="spearman", alternative=alt
            )
            npt.assert_equal(obs, exp)

            obs = mantel(
                self.no_variation, self.no_variation, method="spearman", alternative=alt
            )
            npt.assert_equal(obs, exp)
...
class PairwiseMantelTests(MantelTestData):
    def setUp(self):
        ...
        self.exp_results_dm_dm2 = pd.read_csv(
            get_data_path("pwmantel_exp_results_dm_dm2.txt"), sep="\t", index_col=(0, 1)
        )
    ...
    def test_minimal_compatible_input_with_labels(self):
        ...
        obs = pwmantel(
            self.min_dms, alternative="greater", labels=("minx", "miny", "minz")
        )
        assert_data_frame_almost_equal(obs, self.exp_results_minimal_with_labels)
    ...
    def test_na_p_value(self):
        obs = pwmantel((self.miny_dm, self.minx_dm), method="spearman", permutations=0)
