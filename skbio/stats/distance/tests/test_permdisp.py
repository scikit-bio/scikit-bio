...
class testPERMDISP(TestCase):
    def setUp(self):
        ...
        # when assigned different labels results should be the same
        self.grouping_uneq = ["foo", "foo", "bar", "bar", "bar", "qw", "qw", "qw", "qw"]
    ...
    def test_hdmedians(self):
        exp = np.array(
            [2.01956244, 1.53164546, 2.60571752, 0.91424179, 1.76214416, 1.69943057]
        )
    ...
    def test_confirm_betadispr_results(self):
        ...
        exp_med_mp = pd.Series(
            data=exp_data_m, index=exp_ind, dtype="object", name="PERMDISP results"
        )

        exp_cen_mp = pd.Series(
            data=exp_data_c, index=exp_ind, dtype="object", name="PERMDISP results"
        )
