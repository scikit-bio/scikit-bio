...
class ISubsampleTests(unittest.TestCase):
    ...
    def test_isubsample_simple(self):
        ...
        obs = isubsample(self.mock_sequence_iter(self.sequences), maximum, bin_f=bin_f)
    ...
    def test_per_sample_sequences_min_seqs(self):
        ...
        obs = isubsample(
            self.mock_sequence_iter(self.sequences), maximum, minimum, bin_f=bin_f
        )
        self.assertEqual(sorted(obs, key=lambda x: x[0]), exp)
    ...
    def test_per_sample_sequences_complex(self):
        ...
        obs = isubsample(
            self.mock_sequence_iter(self.sequences), maximum, bin_f=bin_f, buf_size=1
        )
        self.assertEqual(sorted(obs, key=lambda x: x[0]), exp)
