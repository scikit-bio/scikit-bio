...
class BaseTests(TestCase):
    def setUp(self):
        ...
        coord_data = {
            ...
            "PC.635": np.array([-0.277487312135, -0.0295483215975, -0.0744173437992]),
            "PC.356": np.array([0.220886492631, 0.0874848360559, -0.351990132198]),
            "PC.481": np.array([0.0308923744062, -0.0446295973489, 0.133996451689]),
            "PC.354": np.array([0.27616778138, -0.0341866951102, 0.0633000238256]),
            "PC.593": np.array([0.202458130052, -0.115216120518, 0.301820871723]),
            "PC.355": np.array([0.236467470907, 0.21863434374, -0.0301637746424]),
            "PC.607": np.array([-0.105517545144, -0.41405687433, -0.150073017617]),
            "PC.634": np.array([-0.371636765565, 0.115484234741, 0.0721996475289]),
        }
        ...
        self.metadata_map = pd.DataFrame.from_dict(metadata_map, orient="index")
        ...
        gr_wo_msg = GroupResults(
            "Foo",
            np.array([-2.6750, -0.2510, -2.8322, 0.]),
            -1.4398,
...
class GradientTests(BaseTests):
    def test_weight_by_vector(self):
        ...
        trajectory.sort_values(by=0, inplace=True)
        w_vector = pd.Series(np.array([1, 2, 3, 4]), ["s1", "s2", "s3", "s4"]).astype(
            np.float64
        )
        exp = pd.DataFrame.from_dict(
            {
        ...
        w_vector = pd.Series(np.array([50, 52, 55, 57, 60]), sample_ids).astype(
            np.float64
        )
        exp = pd.DataFrame.from_dict(
            {
        ...
        obs = _weight_by_vector(trajectory.loc[sample_ids], w_vector[sample_ids])
        assert_data_frame_almost_equal(obs.sort_index(), exp.sort_index())

    def test_weight_by_vector_single_element(self):
        trajectory = pd.DataFrame.from_dict({"s1": np.array([42])}, orient="index")
    ...
    def test_ANOVA_trajectories(self):
        ...
        exp = CategoryResults("Foo", None, None, "Only one value in the group.")
...
class GradientANOVATests(BaseTests):
    ...
    def test_init_error(self):
        ...
        with self.assertRaises(ValueError):
            GradientANOVA(
                self.coords, self.prop_expl, self.metadata_map, sort_category="foo"
            )

        # Raises ValueError if weighted == True and sort_category == None
        with self.assertRaises(ValueError):
            GradientANOVA(self.coords, self.prop_expl, self.metadata_map, weighted=True)
        ...
        with self.assertRaises(ValueError):
            GradientANOVA(self.coords, self.prop_expl, self.metadata_map, axes=10)

        # Raises ValueError if axes < 0
        with self.assertRaises(ValueError):
            GradientANOVA(self.coords, self.prop_expl, self.metadata_map, axes=-1)
    ...
    def test_normalize_samples(self):
        ...
        subset_metadata_map = pd.DataFrame.from_dict(metadata_map, orient="index")
        ...
        assert_data_frame_almost_equal(bv._coords.sort_index(), exp_coords.sort_index())
    ...
    def test_make_groups_natural_sorting(self):
        ...
        coords = pd.DataFrame.from_dict(
            {
                ...
                "a1": np.array([0.220886492631, 0.0874848360559, -0.351990132198]),
                "a2": np.array([0.0308923744062, -0.0446295973489, 0.133996451689]),
                "a-10": np.array([0.27616778138, -0.0341866951102, 0.0633000238256]),
            },
            orient="index",
        )

        prop_expl = np.array(
            [25.6216900347, 15.7715955926, 14.1215046787, 11.6913885817, 9.83044890697]
        )
        ...
        ga = GradientANOVA(
            coords, prop_expl, df, trajectory_categories=["Col1"], sort_category="Col2"
        )

        exp_groups = {"Col1": {"foo": ["a11.0", "a2", "a-10"], "bar": ["a1", "a10"]}}
...
class AverageGradientANOVATests(BaseTests):
    def test_get_trajectories_all(self):
        ...
        av = AverageGradientANOVA(self.coords, self.prop_expl, self.metadata_map)
    ...
    def test_get_trajectories_weighted(self):
        ...
        exp_control_group = GroupResults(
            "Control",
            np.array(
                [5.7926887872, 4.3242308936, 2.9212403501, 5.5400792151, 1.2326804315]
            ),
            3.9621839355,
            {"avg": 3.9621839355},
            None,
        ...
        exp_treatment = CategoryResults(
            "Treatment", 0.9057666800, [exp_control_group, exp_fast_group], None
        )
...
class TrajectoryGradientANOVATests(BaseTests):
    def test_get_trajectories(self):
        ...
        exp_treatment = CategoryResults(
            "Treatment", 0.9374500147, [exp_control_group, exp_fast_group], None
        )
    ...
    def test_get_trajectories_weighted(self):
        ...
        exp_treatment = CategoryResults(
            "Treatment", 0.6248157720, [exp_control_group, exp_fast_group], None
        )
...
class FirstDifferenceGradientANOVATests(BaseTests):
    def test_get_trajectories(self):
        ...
        exp_treatment = CategoryResults(
            "Treatment", 0.6015260608, [exp_control_group, exp_fast_group], None
        )
    ...
    def test_get_trajectories_weighted(self):
        ...
        exp_treatment = CategoryResults(
            "Treatment", 0.8348644420, [exp_control_group, exp_fast_group], None
        )
...
class WindowDifferenceGradientANOVATests(BaseTests):
    def test_get_trajectories(self):
        ...
        exp_control_group = GroupResults(
            "Control",
            np.array([-2.5790341819, -2.0166764661, -3.0360721941, 0.]),
            -1.9079457105,
        ...
        exp_treatment = CategoryResults(
            "Treatment", 0.0103976830, [exp_control_group, exp_fast_group], None
        )
    ...
    def test_get_trajectories_weighted(self):
        ...
        exp_control_group = GroupResults(
            "Control",
            np.array([-2.6759675112, -0.2510321601, -2.8322876639, 0.]),
            -1.4398218338,
        ...
        exp_treatment = CategoryResults(
            "Treatment", 0.0110675605, [exp_control_group, exp_fast_group], None
        )
