...
class PowerAnalysisTest(TestCase):
    def setUp(self):
        ...
        meta = {
            "GW": {"INT": "N", "ABX": np.nan, "DIV": 19.5, "AGE": "30s", "SEX": "M"},
            "CB": {"INT": "Y", "ABX": np.nan, "DIV": 42.7, "AGE": "30s", "SEX": "M"},
            "WM": {"INT": "N", "ABX": "N", "DIV": 27.5, "AGE": "20s", "SEX": "F"},
            "MH": {"INT": "Y", "ABX": "N", "DIV": 62.3, "AGE": "30s", "SEX": "F"},
            "CD": {"INT": "Y", "ABX": "Y", "DIV": 36.4, "AGE": "40s", "SEX": "F"},
            "LF": {"INT": "Y", "ABX": "N", "DIV": 50.2, "AGE": "20s", "SEX": "M"},
            "PP": {"INT": "N", "ABX": "Y", "DIV": 10.8, "AGE": "30s", "SEX": "F"},
            "MM": {"INT": "N", "ABX": "N", "DIV": 55.6, "AGE": "40s", "SEX": "F"},
            "SR": {"INT": "N", "ABX": "Y", "DIV": 2.2, "AGE": "20s", "SEX": "M"},
            "TS": {"INT": "N", "ABX": "Y", "DIV": 16.1, "AGE": "40s", "SEX": "M"},
            "PC": {"INT": "Y", "ABX": "N", "DIV": 82.6, "AGE": "40s", "SEX": "M"},
            "NR": {"INT": "Y", "ABX": "Y", "DIV": 15.7, "AGE": "20s", "SEX": "F"},
        }
    ...
    def test_subsample_power_defaults(self):
        test_p, test_c = subsample_power(self.f, self.pop, num_iter=10, num_runs=5)
    ...
    def test_subsample_power_matches(self):
        test_p, test_c = subsample_power(
            self.f, samples=self.pop, num_iter=10, num_runs=5, draw_mode="matched"
        )
        self.assertEqual(test_p.shape, (5, 4))
        npt.assert_array_equal(np.array([10, 20, 30, 40]), test_c)
    ...
    def test_subsample_power_multi_p(self):
        test_p, test_c = subsample_power(
            lambda x: np.array([0.5, 0.5]), samples=self.pop, num_iter=10, num_runs=5
        )
    ...
    def test__get_min_size_strict(self):
        known = 5
        test = _get_min_size(self.meta, "INT", ["ABX", "SEX"], ["Y", "N"], True)
        self.assertEqual(test, known)

    def test__get_min_size_relaxed(self):
        known = 5
        test = _get_min_size(self.meta, "INT", ["ABX", "SEX"], ["Y", "N"], False)
    ...
    def test__compare_distributions_sample_counts_error(self):
        with self.assertRaises(ValueError):
            _compare_distributions(self.f, [self.pop[0][:5], self.pop[1]], 1, counts=25)
    ...
    def test__compare_distributions_draw_mode(self):
        draw_mode = "Ultron"
        with self.assertRaises(ValueError):
            _check_subsample_power_inputs(self.f, self.pop, draw_mode, self.num_p)
    ...
    def test__compare_distributions_multiple_returns(self):
        ...
        test = _compare_distributions(f, self.pop, 3, mode="matched", num_iter=3)
        npt.assert_array_equal(known, test)

    def test_check_subsample_power_inputs_matched_mode(self):
        with self.assertRaises(ValueError):
            _check_subsample_power_inputs(
                self.f, samples=[np.ones((2)), np.ones((5))], draw_mode="matched"
            )
    ...
    def test_check_subsample_power_inputs_ratio(self):
        with self.assertRaises(ValueError):
            _check_subsample_power_inputs(self.f, self.samps, ratio=np.array([1, 2, 3]))
    ...
    def test__calculate_power_curve_default(self):
        ...
        test = _calculate_power_curve(self.f, self.pop, self.num_samps, num_iter=100)
    ...
    def test__calculate_power_curve_alpha(self):
        ...
        known = np.array([0.31, 0.568, 0.842, 0.954, 0.995, 1.000, 1.000, 1.000, 1.000])
    ...
    def test__calculate_power_curve_ratio(self):
        ...
        # Generates the test values
        test = _calculate_power_curve(
            self.f, self.pop, self.num_samps, ratio=np.array([0.25, 0.75]), num_iter=100
        )
    ...
    def test_paired_subsample_undefined(self):
        ...
        test_array = paired_subsamples(self.meta, cat, control_cats, order=order)
    ...
    def test_paired_subsamples_not_strict(self):
        known_array = [{"WM", "MM", "GW", "SR", "TS"}, {"LF", "PC", "CB", "NR", "CD"}]

        # Gets the test values
        cat = "INT"
        control_cats = ["ABX", "AGE"]
        test_array = paired_subsamples(self.meta, cat, control_cats, strict_match=False)
    ...
    def test__identify_sample_groups(self):
        ...
        test_pairs, test_index = _identify_sample_groups(
            self.meta, "INT", ["SEX", "AGE"], order=["N", "Y"], strict_match=True
        )
        self.assertEqual(known_pairs.keys(), test_pairs.keys())
        self.assertEqual(sorted(known_pairs.values()), sorted(test_pairs.values()))
        npt.assert_array_equal(known_index, test_index)

    def test__identify_sample_groups_not_strict(self):
        # Defines the know values
        known_pairs = {
            1: [np.array(["PP"], dtype=object), np.array(["CD", "NR"], dtype=object)],
            0: [np.array(["MM", "WM"], dtype=object), np.array(["MH"], dtype=object)],
            2: [np.array(["GW"], dtype=object), np.array(["CB"], dtype=object)],
        }
        known_index = np.array([0, 1, 2])
        test_pairs, test_index = _identify_sample_groups(
            self.meta, "INT", ["SEX", "ABX"], order=["N", "Y"], strict_match=False
        )
        self.assertEqual(known_pairs.keys(), test_pairs.keys())
    ...
    def test__draw_paired_samples(self):
        ...
        test_samps = _draw_paired_samples(self.meta_pairs, self.pair_index, num_samps)
