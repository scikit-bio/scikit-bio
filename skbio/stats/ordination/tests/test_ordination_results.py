...
class TestOrdinationResults(unittest.TestCase):
    def setUp(self):
        ...
        samples_df = pd.DataFrame(samples, index=sample_ids, columns=["CA1", "CA2"])
        features_df = pd.DataFrame(features, index=features_ids, columns=["CA1", "CA2"])
...
class TestOrdinationResultsPlotting(unittest.TestCase):
    def setUp(self):
        ...
        self.df = pd.DataFrame(
            [["foo", "42", 10], [22, 0, 8], [22, -4.2, np.nan], ["foo", "42.19", 11]],
            index=["A", "B", "C", "D"],
        ...
        samples_df = pd.DataFrame(samples, ["A", "B", "C", "D"], ["PC1", "PC2", "PC3"])
    ...
    def test_plot_with_numeric_metadata_and_plot_options(self):
        ...
        self.check_basic_figure_sanity(fig, 2, "a title", False, "PC 2", "PC 1", "PC 3")

    def test_plot_with_categorical_metadata_and_plot_options(self):
        fig = self.min_ord_results.plot(
            self.df, "categorical", axes=[2, 0, 1], title="a title", cmap="Accent"
        )
    ...
    def test_plot_with_invalid_axis_labels(self):
        with self.assertRaisesRegex(ValueError, r"axis_labels.*4"):
            self.min_ord_results.plot(axes=[2, 0, 1], axis_labels=("a", "b", "c", "d"))
    ...
    def test_validate_plot_axes_invalid_input(self):
        ...
        with self.assertRaisesRegex(ValueError, r"exactly three.*found 4"):
            self.min_ord_results._validate_plot_axes(coord_matrix, (0, 1, 2, 3))
    ...
    def test_get_plot_point_colors_no_df_or_column(self):
        obs = self.min_ord_results._get_plot_point_colors(None, None, ["B", "C"], "jet")
    ...
    def test_get_plot_point_colors_categorical_column(self):
        # subset of the ids in df
        exp_colors = [[0.0, 0.0, 0.5, 1.0], [0.0, 0.0, 0.5, 1.0], [0.5, 0.0, 0.0, 1.0]]
        exp_color_dict = {"foo": [0.5, 0.0, 0.0, 1.0], 22: [0.0, 0.0, 0.5, 1.0]}
        obs = self.min_ord_results._get_plot_point_colors(
