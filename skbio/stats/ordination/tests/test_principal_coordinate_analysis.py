...
class TestPCoA(TestCase):
    ...
    def test_fsvd(self):
        ...
        results = pcoa(dm2, method="fsvd", number_of_dimensions=3, inplace=False)

        results_inplace = pcoa(dm2, method="fsvd", number_of_dimensions=3, inplace=True)
        ...
        results2 = pcoa(dm3, method="fsvd", number_of_dimensions=0, inplace=False)
        expected_results2 = pcoa(
            dm3, method="fsvd", number_of_dimensions=dm3.data.shape[0], inplace=False
        )
        ...
        with self.assertWarnsRegex(RuntimeWarning, "no value for number_of_dimensions"):
            pcoa(dm_big, method="fsvd", number_of_dimensions=0)

    def test_extensive(self):
        eigvals = [0.3984635, 0.36405689, 0.28804535, 0.27479983, 0.19165361, 0.0]
        proportion_explained = [
            0.2626621381,
            0.2399817314,
            0.1898758748,
        ...
        samples = [
            ...
            [0.37494056, 0.22334055, -0.20892914, 0.05057395, -0.18710366, 0.0],
            [-0.33517593, -0.23855979, -0.3099887, 0.11521787, -0.05021553, 0.0],
            [0.25412394, -0.4123464, 0.23343642, 0.06403168, -0.00482608, 0.0],
            [-0.28256844, 0.18606911, 0.28875631, -0.06455635, -0.21141632, 0.0],
        ...
        expected_results = OrdinationResults(
            ...
            long_method_name="Principal Coordinate Analysis",
            eigvals=pd.Series(eigvals, index=axis_labels),
            samples=pd.DataFrame(samples, index=sample_ids, columns=axis_labels),
            proportion_explained=pd.Series(proportion_explained, index=axis_labels),
        )
