...
def _estimate_bias_em(beta, var_hat, tol=1e-5, max_iter=100):
    ...
    while loss > tol and epoch < max_iter:
        ...
        updated[6] = minimize(func, params[6], args=(means[1], resp[1]), **OPTARG).x[0]
        updated[7] = minimize(func, params[7], args=(means[2], resp[2]), **OPTARG).x[0]
...
def _adjust_variances(var_hat, vcov_hat, var_delta, s0_perc):
    ...
    Notes
    -----
    This function will update `var_hat` and `vcov_hat` in place.
    ...
    diag_idx = np.arange(var_hat.shape[1])
    vcov_hat[:, diag_idx, diag_idx] = var_hat
...
def _ancombc(
    ...
    if not reestimate:
        ...
        # Calculate statistics
        se_hat, W, pval, qval, reject = _calc_statistics(
            beta_hat, var_hat, alpha, p_adjust
        )
        ...
        features = res_main["features"]
        covariates = res_main["covariates"]
        n_tax = len(features)
        ...
        reject = res_main["reject"]

        res_dict = {
            "FeatureID": [x for x in features for _ in range(n_cov)],
            "Covariate": list(covariates) * n_tax,
            "Log2(FC)": beta_hat.ravel(),
            "SE": se_hat.ravel(),
            "W": W.ravel(),
            "pvalue": p_hat.ravel(),
