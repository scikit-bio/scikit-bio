...
class ExampleMotifsTester(ExampleGrammaredSequence):
    ...
    def _motifs(self):
        ...
        return {"name1": lambda x, _, __: str(x), "name2": lambda x, _, __: len(x)}
...
class TestGrammaredSequence(TestCase):
    ...
    def test_gap_chars_and_degenerates_share(self):
        with self.assertRaisesRegex(
            ...
            class GrammaredSequenceGapInDegenerateMap(ExampleGrammaredSequence):
                @classproperty
                def degenerate_map(cls):
                    return {"X": set("AB")}
    ...
    def test_init_lowercase_all_lowercase(self):
        ...
        with self.assertRaisesRegex(ValueError, "\['a', 'b', 'c', 'x', 'y', 'z'\]"):
    ...
    def test_nondegenerate_chars(self):
        expected = set("ABC")
        self.assertEqual(ExampleGrammaredSequence.nondegenerate_chars, expected)
    ...
    def test_gaps(self):
        ...
        self.assertIs(ExampleGrammaredSequence("").gaps().dtype, np.dtype("bool"))
        npt.assert_equal(
            ExampleGrammaredSequence("ABCXBZYABC").gaps(), np.zeros(10).astype(bool)
        )
    ...
    def test_degenerates(self):
        self.assertIs(type(ExampleGrammaredSequence("").degenerates()), np.ndarray)
        self.assertIs(
        ...
        npt.assert_equal(
            ExampleGrammaredSequence("ZYZYZ").degenerates(), np.ones(5).astype(bool)
        )
    ...
    def test_has_degenerates(self):
        self.assertIs(type(ExampleGrammaredSequence("").has_degenerates()), bool)
        self.assertIs(type(ExampleGrammaredSequence("X").has_degenerates()), bool)
    ...
    def test_nondegenerates(self):
        self.assertIs(type(ExampleGrammaredSequence("").nondegenerates()), np.ndarray)
        self.assertIs(
            ExampleGrammaredSequence("").nondegenerates().dtype, np.dtype("bool")
        )
        ...
        npt.assert_equal(
            ExampleGrammaredSequence("ABABA").nondegenerates(), np.ones(5).astype(bool)
        )
    ...
    def test_definites(self):
        self.assertIs(type(ExampleGrammaredSequence("").definites()), np.ndarray)
        self.assertIs(ExampleGrammaredSequence("").definites().dtype, np.dtype("bool"))
        ...
        npt.assert_equal(
            ExampleGrammaredSequence("ABABA").definites(), np.ones(5).astype(bool)
        )
    ...
    def test_has_nondegenerates(self):
        self.assertIs(type(ExampleGrammaredSequence("").has_nondegenerates()), bool)
        self.assertIs(type(ExampleGrammaredSequence("A").has_nondegenerates()), bool)

        self.assertFalse(ExampleGrammaredSequence("").has_nondegenerates())
        self.assertFalse(ExampleGrammaredSequence("X-.YZ").has_nondegenerates())

        self.assertTrue(ExampleGrammaredSequence("C").has_nondegenerates())
        self.assertTrue(ExampleGrammaredSequence(".XYZ-ABC").has_nondegenerates())

    def test_has_definites(self):
        self.assertIs(type(ExampleGrammaredSequence("").has_definites()), bool)
        self.assertIs(type(ExampleGrammaredSequence("A").has_definites()), bool)
    ...
    def test_degap(self):
        kw = {"metadata": {"id": "some_id", "description": "some description"}}

        self.assertEqual(
            ExampleGrammaredSequence(
                "", positional_metadata={"qual": []}, **kw
            ).degap(),
            ExampleGrammaredSequence("", positional_metadata={"qual": []}, **kw),
        )
        ...
        self.assertEqual(
            ...
            ExampleGrammaredSequence(
                "ABCXYZ", positional_metadata={"qual": [0, 1, 2, 4, 5, 6]}, **kw
            ),
        )
        ...
        self.assertEqual(
            ...
            ExampleGrammaredSequence(
                "ABCXYZ", positional_metadata={"qual": [2, 3, 4, 6, 7, 8]}, **kw
            ),
        )

        self.assertEqual(
            ExampleGrammaredSequence(
                "---.-.-.-.-.", positional_metadata={"quality": np.arange(12)}, **kw
            ).degap(),
            ExampleGrammaredSequence(
                "", positional_metadata={"quality": np.array([], dtype=np.int64)}, **kw
            ),
        )
    ...
    def test_expand_degenerates_all_degens(self):
        ...
        obs = sorted(ExampleGrammaredSequence("XYZ").expand_degenerates(), key=str)
    ...
    def test_to_regex_with_degens(self):
        ...
        regex = seq.to_regex()
        self.assertFalse(any(regex.match(s) is None for s in "ABA ABC ACA ACC".split()))
        self.assertTrue(all(regex.match(s) is None for s in "CBA BBA ABB AAA".split()))
