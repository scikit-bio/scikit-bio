...
def subsample_power(
    ...
    `skbio.power.subsample_power` to compare the two groups. If we had
    metadata about other risk factors, like a reproductive history, BMI,
    tobacco use, we might want to use
    `skbio.power.subsample_paired_power`.
...
def subsample_paired_power(
    ...
    >>> from skbio.power import subsample_paired_power
...
def paired_subsamples(meta, cat, control_cats, order=None, strict_match=True):
    ...
    >>> from skbio.power import paired_subsamples
