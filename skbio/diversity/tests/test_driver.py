...
class AlphaDiversityTests(TestCase):
    ...
    def test_invalid_input(self):
        ...
        self.assertRaises(
            ValueError, alpha_diversity, "observed_otus", self.table1, list("ABC")
        )
        ...
        self.assertRaises(ValueError, alpha_diversity, "observed_otus", [[[43]]])

        # negative counts
        self.assertRaises(ValueError, alpha_diversity, "observed_otus", [0, 3, -12, 42])

        # additional kwargs
        self.assertRaises(
            TypeError, alpha_diversity, "observed_otus", [0, 1], not_a_real_kwarg=42.0
        )
        self.assertRaises(
            TypeError,
            alpha_diversity,
    ...
    def test_invalid_input_phylogenetic(self):
        ...
        otu_ids = ["OTU1", "OTU2", "OTU3"]
        self.assertRaises(
            ValueError, alpha_diversity, "faith_pd", counts, otu_ids=otu_ids, tree=t
        )
    ...
    def test_empty(self):
        ...
        actual = alpha_diversity(
            "faith_pd", np.array([], dtype=np.int64), tree=self.tree1, otu_ids=[]
        )
        expected = pd.Series([0.])
        assert_series_almost_equal(actual, expected)
    ...
    def test_single_count_vector(self):
        ...
        actual = alpha_diversity(
            "faith_pd", np.array([1, 3, 0, 1, 0]), tree=self.tree1, otu_ids=self.oids1
        )
        self.assertAlmostEqual(actual[0], 4.5)

    def test_input_types(self):
        list_result = alpha_diversity("observed_otus", [1, 3, 0, 1, 0])
        array_result = alpha_diversity("observed_otus", np.array([1, 3, 0, 1, 0]))
...
class TestPartialBetaDiversity(TestCase):
    ...
    def test_pairs_not_subset(self):
        ...
        with self.assertRaisesRegex(ValueError, error_msg):
            partial_beta_diversity(
                (lambda x, y: x + y), self.table1, self.sids1, id_pairs=[("x", "b")]
            )
