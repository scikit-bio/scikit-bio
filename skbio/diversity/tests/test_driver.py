...
from skbio.diversity._driver import _qualitative_beta_metrics, _valid_beta_metrics


class AlphaDiversityTests(TestCase):
    def setUp(self):
        self.table1 = np.array(
            [[1, 3, 0, 1, 0], [0, 2, 0, 4, 4], [0, 0, 6, 2, 1], [0, 0, 1, 1, 1]]
        )
    ...
    def test_invalid_input(self):
        ...
        self.assertRaises(ValueError, alpha_diversity, "sobs", self.table1, list("ABC"))

        # unknown metric provided
        self.assertRaises(ValueError, alpha_diversity, "not-a-metric", self.table1)
    ...
    def test_invalid_input_phylogenetic(self):
        ...
        taxa = ["OTU1", "OTU2", "OTU3"]
        self.assertRaises(
            DuplicateNodeError, alpha_diversity, "faith_pd", counts, taxa=taxa, tree=t
        )

        # unrooted tree as input
        t = TreeNode.read(io.StringIO("((OTU1:0.1, OTU2:0.2):0.3, OTU3:0.5,OTU4:0.7);"))
        counts = [1, 2, 3]
        taxa = ["OTU1", "OTU2", "OTU3"]
        self.assertRaises(
            ValueError, alpha_diversity, "faith_pd", counts, taxa=taxa, tree=t
        )
        ...
        self.assertRaises(
            MissingNodeError, alpha_diversity, "faith_pd", counts, taxa=taxa, tree=t
        )
    ...
    def test_empty(self):
        ...
        actual = alpha_diversity(
            "faith_pd", np.array([], dtype=np.int64), tree=self.tree1, taxa=[]
        )
        expected = pd.Series([0.0])
        assert_series_almost_equal(actual, expected)
    ...
    def test_single_count_vector(self):
        ...
        actual = alpha_diversity(
            "faith_pd", np.array([1, 3, 0, 1, 0]), tree=self.tree1, taxa=self.oids1
        )
    ...
    def test_input_types(self):
        ...
        list_result = alpha_diversity(
            "faith_pd", [1, 3, 0, 1, 0], tree=self.tree1, taxa=self.oids1
        )
    ...
    def test_faith_pd(self):
        ...
        actual = alpha_diversity(
            "faith_pd", self.table1, tree=self.tree1, taxa=self.oids1
        )
        assert_series_almost_equal(actual, expected)
        ...
        expected = pd.Series(expected)
        actual = alpha_diversity(
            "faith_pd", self.table2, tree=self.tree2, taxa=self.oids2
        )
        assert_series_almost_equal(actual, expected)
    ...
    def test_phydiv(self):
        ...
        expected = pd.Series(expected)
        actual = alpha_diversity(
            "phydiv", self.table1, tree=self.tree1, taxa=self.oids1
        )
        assert_series_almost_equal(actual, expected)

        expected = []
        for e in self.table1:
            expected.append(phydiv(e, tree=self.tree1, taxa=self.oids1, rooted=False))
        expected = pd.Series(expected)
        actual = alpha_diversity(
            "phydiv", self.table1, tree=self.tree1, taxa=self.oids1, rooted=False
        )
        assert_series_almost_equal(actual, expected)

        expected = []
        for e in self.table1:
            expected.append(phydiv(e, tree=self.tree1, taxa=self.oids1, weight=True))
        expected = pd.Series(expected)
        actual = alpha_diversity(
            "phydiv", self.table1, tree=self.tree1, taxa=self.oids1, weight=True
        )
        assert_series_almost_equal(actual, expected)
...
class BetaDiversityTests(TestCase):
    ...
    def test_available_metrics(self):
        for metric in _valid_beta_metrics:
            try:
                ...
                raise ValueError(f"Metric {metric} failed with exception:\n {exc}")
    ...
    def test_invalid_input(self):
        ...
        with self.assertRaisesRegex(TypeError, error_msg):
            beta_diversity("euclidean", [[0, 1, 3], [0, 3, 12]], not_a_real_kwarg=42.0)
        with self.assertRaisesRegex(TypeError, error_msg):
            beta_diversity(
                "unweighted_unifrac",
                [[0, 1, 3], [0, 3, 12]],
    ...
    def test_invalid_input_phylogenetic(self):
        ...
        taxa = ["OTU1", "OTU2", "OTU3"]
        self.assertRaises(
            ValueError, beta_diversity, "weighted_unifrac", counts, taxa=taxa, tree=t
        )
        self.assertRaises(
            ValueError, beta_diversity, "unweighted_unifrac", counts, taxa=taxa, tree=t
        )
    ...
    def test_input_types(self):
        ...
        actual_list = beta_diversity("euclidean", [[1, 5], [2, 3]], ids=["a", "b"])
    ...
    def test_euclidean(self):
        ...
        for id1 in self.sids2:
            for id2 in self.sids2:
                npt.assert_almost_equal(actual_dm[id1, id2], expected_dm[id1, id2], 6)
    ...
    def test_unweighted_unifrac(self):
        ...
        for id1 in self.sids1:
            for id2 in self.sids1:
                npt.assert_almost_equal(dm1[id1, id2], expected_dm[id1, id2], 6)
    ...
    def test_weighted_unifrac(self):
        ...
        dm2 = beta_diversity(
            weighted_unifrac, self.table1, self.sids1, taxa=self.oids1, tree=self.tree1
        )
        self.assertEqual(dm1.shape, (3, 3))
        self.assertEqual(dm1, dm2)
        expected_data = [
    ...
    def test_alt_pairwise_func(self):
        ...
        dm1 = beta_diversity("euclidean", self.table1, pairwise_func=not_a_real_pdist)
        expected = DistanceMatrix([[0.0, 42.0], [42.0, 0.0]])
        self.assertEqual(dm1, expected)
