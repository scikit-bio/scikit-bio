...
def create_fake_observation():
    ...
    fake_observation = subsample(counts, 1000)
    exp_p = 1 - sum( [ x/total for (x,y) in zip(counts, fake_observation) if y > 0] )

    return fake_observation, exp_p
...
def test_lladser_ci_nan():
    """lladser_ci estimate returns nan if sample is too short and no estimate can be made"""
...
def test_lladser_ci_f2(): 
    ...
    # of the interval in 38% of all test runs.
    # To make this test pass reliable we thus have to set the threshold much lower
    # and do more reps. For 0.93  and 1000 reps, the binomial distribution tells us
    # that the test will pass more often than 99%.   
    reps = 1000
    sum = 0
    for i in range(reps):
