...
class TreeTests(TestCase):
    def setUp(self):
        ...
        self.complex_tree = TreeNode.read(
            io.StringIO("(((a,b)int1,(x,y,(w,z)int2,(c,d)int3)int4),(e,f)int5);")
        )
    ...
    def test_observed_node_counts(self):
        ...
        self.assertEqual(self.simple_t.observed_node_counts(taxon_counts), expected)
        # error on zero count(s)
        taxon_counts = {"a": 0}
        self.assertRaises(ValueError, self.simple_t.observed_node_counts, taxon_counts)
        ...
        self.assertRaises(
            MissingNodeError, self.simple_t.observed_node_counts, taxon_counts
        )
        ...
        taxon_counts = {}
        expected = defaultdict(int)
        self.assertEqual(self.complex_tree.observed_node_counts(taxon_counts), expected)
    ...
    def test_copy(self):
        ...
        gen = zip(
            cp.traverse(include_self=True), self.simple_t.traverse(include_self=True)
        )
    ...
    def test_subsets(self):
        ...
        t = self.simple_t
        self.assertEqual(t.subsets(), frozenset([frozenset("ab"), frozenset("cd")]))
    ...
    def test_accumulate_to_ancestor(self):
        ...
        t = TreeNode.read(io.StringIO("((a:0.1,b:0.2)c:0.3,(d:0.4,e)f:0.5)root;"))
        a = t.find("a")
        b = t.find("b")
        exp_to_root = 0.1 + 0.3
    ...
    def test_tip_tip_distances_endpoints(self):
        ...
        names = ["H", "G", "M"]
        exp = DistanceMatrix(
            np.array([[0, 2.0, 6.7], [2.0, 0, 6.7], [6.7, 6.7, 0.0]]), ["H", "G", "M"]
        )
    ...
    def test_index_tree(self):
        ...
        nodes_1 = [n.id for n in t1.traverse(self_before=False, self_after=True)]
        ...
        nodes_2 = [n.id for n in t2.traverse(self_before=False, self_after=True)]
        self.assertEqual(nodes_2, [0, 1, 4, 2, 3, 5, 8, 6, 7, 9, 10])
        npt.assert_equal(
            child_2, np.array([[4, 0, 1], [5, 2, 3], [8, 4, 5], [9, 6, 7], [10, 8, 9]])
        )

        # test for third tree: contains trifurcation and single-child parent
        id_3, child_3 = t3.index_tree()
        nodes_3 = [n.id for n in t3.traverse(self_before=False, self_after=True)]
        self.assertEqual(nodes_3, [0, 1, 2, 4, 3, 5, 8, 6, 7, 9, 10])
        npt.assert_equal(
            child_3, np.array([[4, 0, 2], [5, 3, 3], [8, 4, 5], [9, 6, 7], [10, 8, 9]])
        )
    ...
    def test_descending_branch_length_bug_1847(self):
        tr = TreeNode.read(
            io.StringIO("(((A:.1,B:1.2)C:.6,(D:.9,E:.6)F:.9)G:2.4,(H:.4,I:.5)J:1.3)K;")
        )
        tr.length = 1
    ...
    def test_descending_branch_length(self):
        ...
        tr = TreeNode.read(
            io.StringIO("(((A,B:1.2)C:.6,(D:.9,E:.6)F:.9)G:2.4,(H:.4,I:.5)J:1.3)K;")
        )
        tdbl = tr.descending_branch_length()
        npt.assert_almost_equal(tdbl, 8.8)

        tr = TreeNode.read(
            io.StringIO("(((A,B:1.2)C:.6,(D:.9,E:.6)F)G:2.4,(H:.4,I:.5)J:1.3)K;")
        )
        tdbl = tr.descending_branch_length()
    ...
    def test_to_array(self):
        ...
        exp = np.array(["a", "b", "c", "d", "x", "y", "e", "f", "z", "z", None])
        obs = arrayed["name"]
        npt.assert_equal(obs, exp)
    ...
    def test_to_array_nan_length_value(self):
        ...
        npt.assert_equal(indexed["length"], np.array([1, 2, 3, np.nan], dtype=float))
        indexed = t.to_array(nan_length_value=0.0)
        npt.assert_equal(indexed["length"], np.array([1, 2, 3, 0.0], dtype=float))
        indexed = t.to_array(nan_length_value=42.0)
        npt.assert_equal(indexed["length"], np.array([1, 2, 3, 42.0], dtype=float))

        t = TreeNode.read(io.StringIO("((a:1, b:2)c:3)root:4;"))
        indexed = t.to_array(nan_length_value=42.0)
        npt.assert_equal(indexed["length"], np.array([1, 2, 3, 4], dtype=float))

        t = TreeNode.read(io.StringIO("((a:1, b:2)c)root;"))
        indexed = t.to_array(nan_length_value=42.0)
        npt.assert_equal(indexed["length"], np.array([1, 2, 42.0, 42.0], dtype=float))
    ...
    def test_from_taxonomy(self):
        ...
        exp = TreeNode.read(
            io.StringIO(
                "((((((((1)g)f)e)d,((((2)y)x)))c)b)a," "(((((((3,5)n,(4)q)m)l)k)j)i)h);"
            )
        )
    ...
    def test_from_taxdump(self):
        ...
        names = pd.DataFrame(
            [
                ...
                [118884, "unclassified Gammaproteobacteria", np.nan, "scientific name"],
        ...
        self.assertListEqual(
            [x.name for x in node.children], ["Escherichia", "Klebsiella", "Shigella"]
        )
        ...
        names = names[names["name_class"] == "scientific name"]["name_txt"].to_dict()
        obs = TreeNode.from_taxdump(nodes, names)
        self.assertEqual(obs.count(), 18)
        self.assertEqual(obs.name, "root")
        ...
        with self.assertRaises(ValueError) as ctx:
            TreeNode.from_taxdump(nodes)
        self.assertEqual(str(ctx.exception), "There are more than one top-level node.")
