...
from skbio.metadata._testing import MetadataMixinTests, PositionalMetadataMixinTests
...
class TestTabularMSAMetadata(unittest.TestCase, ReallyEqualMixin, MetadataMixinTests):
    def setUp(self):
...
class TestTabularMSA(unittest.TestCase, ReallyEqualMixin):
    ...
    def test_from_dict_multiple_sequences(self):
        msa = TabularMSA.from_dict({1: DNA("ACG"), 2: DNA("GGG"), 3: DNA("TAG")})
        # Sort because order is arbitrary.
        msa.sort()
        self.assertEqual(
            msa, TabularMSA([DNA("ACG"), DNA("GGG"), DNA("TAG")], index=[1, 2, 3])
        )
    ...
    def test_constructor_not_monomorphic(self):
        ...
        with self.assertRaisesRegex(TypeError, "matching type.*float.*Protein"):
            TabularMSA([Protein(""), Protein(""), 42.0, Protein("")])

    def test_constructor_unequal_length(self):
        with self.assertRaisesRegex(
            ValueError, "must match the number of positions.*1 != 0"
        ):
            TabularMSA([Protein(""), Protein("P")])

        with self.assertRaisesRegex(
            ValueError, "must match the number of positions.*1 != 3"
        ):
    ...
    def test_constructor_minter_and_index_both_provided(self):
        with self.assertRaisesRegex(ValueError, "both.*minter.*index"):
            TabularMSA([DNA("ACGT"), DNA("TGCA")], minter=str, index=["a", "b"])
    ...
    def test_constructor_missing_minter_metadata_key(self):
        with self.assertRaises(KeyError):
            TabularMSA(
                [DNA("ACGT", metadata={"foo": "bar"}), DNA("TGCA")], minter="foo"
            )
    ...
    def test_constructor_index_length_mismatch_iterable(self):
        with self.assertRaisesRegex(ValueError, "sequences.*2.*index length.*0"):
    ...
    def test_constructor_empty_with_labels(self):
        ...
        # position empty
        msa = TabularMSA(
            [DNA("", metadata={"id": 42}), DNA("", metadata={"id": 43})], minter="id"
        )
    ...
    def test_constructor_with_multiindex_index(self):
        msa = TabularMSA([DNA("AA"), DNA("GG")], index=[("foo", 42), ("bar", 43)])
    ...
    def test_index_mixed_type(self):
        msa = TabularMSA([DNA("AC"), DNA("CA"), DNA("AA")], index=["abc", "d", 42])
    ...
    def test_index_setter_non_unique_index(self):
        ...
        self.assertEqual(msa, TabularMSA([RNA("UUU"), RNA("AAA")], index=["1", "1"]))
    ...
    def test_bool(self):
        ...
        self.assertFalse(
            TabularMSA(
                [RNA("", metadata={"id": 1}), RNA("", metadata={"id": 2})], minter="id"
            )
        )
    ...
    def test_eq_and_ne(self):
        ...
        components = [
            ...
            ([RNA("AG"), RNA("GA")], {"minter": lambda x: "AG" if "AG" in x else "GG"}),
            ...
            ([RNA("AG"), RNA("GG")], {"positional_metadata": {"foo": [42, 43]}}),
            ([RNA("AG"), RNA("GG")], {"positional_metadata": {"foo": [43, 44]}}),
            (
                [RNA("AG"), RNA("GG")],
        ...
        for (seqs1, kwargs1), (seqs2, kwargs2) in itertools.combinations(components, 2):
            obj1 = TabularMSA(seqs1, **kwargs1)
            obj2 = TabularMSA(seqs2, **kwargs2)
            self.assertReallyNotEqual(obj1, obj2)
            self.assertReallyNotEqual(obj1, TabularMSASubclass(seqs2, **kwargs2))
    ...
    def test_eq_ignores_minter_str_and_lambda(self):
        ...
        msa2 = TabularMSA(
            [DNA("ACGT", metadata={"id": "a"})], minter=lambda x: x.metadata["id"]
        )
        self.assertReallyEqual(msa1, msa2)
    ...
    def test_eq_default_index_and_equivalent_provided_index(self):
        ...
        msa2 = TabularMSA([DNA("ACGT"), DNA("----"), DNA("....")], index=[0, 1, 2])
    ...
    def test_reassign_index_non_empty(self):
        msa = TabularMSA(
            [DNA("ACG", metadata={"id": 1}), DNA("AAA", metadata={"id": 2})], minter=str
        )
    ...
    def test_reassign_index_mapping_invalid_type(self):
        ...
        with self.assertRaisesRegex(TypeError, "mapping.*dict.*callable.*list"):
    ...
    def test_reassign_index_to_multiindex_with_minter(self):
        ...
        self.assertIsInstance(msa.index, pd.MultiIndex)
        self.assertEqual(
            msa, TabularMSA([DNA("AC"), DNA(".G")], index=[("foo", 42), ("bar", 43)])
        )
    ...
    def test_sort_on_unorderable_msa_index(self):
        msa = TabularMSA([DNA("AAA"), DNA("ACG"), DNA("---")], index=[42, 41, "foo"])
        with self.assertRaises(TypeError):
            msa.sort()
        self.assertEqual(
            msa, TabularMSA([DNA("AAA"), DNA("ACG"), DNA("---")], index=[42, 41, "foo"])
    ...
    def test_sort_multiple_sequences_on_msa_index(self):
        msa = TabularMSA([DNA("TC"), DNA("GG"), DNA("CC")], index=["z", "a", "b"])
        msa.sort(ascending=True)
        self.assertEqual(
            msa, TabularMSA([DNA("GG"), DNA("CC"), DNA("TC")], index=["a", "b", "z"])
        )

        msa = TabularMSA([DNA("TC"), DNA("GG"), DNA("CC")], index=["z", "a", "b"])
        msa.sort(ascending=False)
        self.assertEqual(
            msa, TabularMSA([DNA("TC"), DNA("CC"), DNA("GG")], index=["z", "b", "a"])
        )
    ...
    def test_sort_multiindex_with_level(self):
        ...
        msa.sort(level=1)
        self.assertEqual(
            msa, TabularMSA([DNA("A"), DNA("G"), DNA("C")], index=second_sorted)
        )
...
class SharedIndexTests:
    ...
    def test_bool_index_too_big(self):
        ...
        with self.assertRaises(IndexError):
            self.get(
                msa, ([True, False, True, False], [True, False, True, False, False])
            )
...
class SharedPropertyIndexTests(SharedIndexTests):
    def setUp(self):
        self.combo_msa = TabularMSA(
            [
                DNA("ACGTA", metadata={0: 0}, positional_metadata={0: [1, 2, 3, 4, 5]}),
                DNA("CGTAC", metadata={1: 1}, positional_metadata={1: [1, 2, 3, 4, 5]}),
                DNA("GTACG", metadata={2: 2}, positional_metadata={2: [1, 2, 3, 4, 5]}),
                DNA("TACGT", metadata={3: 3}, positional_metadata={3: [1, 2, 3, 4, 5]}),
                DNA("ACGTT", metadata={4: 4}, positional_metadata={4: [1, 2, 3, 4, 5]}),
            ],
            index=list("ABCDE"),
            metadata={"x": "x"},
        ...
        for idx1, exp1 in self.combo_first_axis:
            for idx2, exp2 in self.combo_second_axis:
                self.assertEqual(
                    ...
                    msg=("%r did not match iloc[%r, %r]" % ((idx1, idx2), exp1, exp2)),
                )
...
class TestLoc(SharedPropertyIndexTests, unittest.TestCase):
    ...
    def test_multiindex_complicated_axis(self):
        ...
        self.assertEqual(
            self.get(msa, (([False, True, False, True], "x", 0), Ellipsis)),
            TabularMSA(
                [d],
                metadata={"x": "y"},
                positional_metadata={"c": ["a", "b", "c", "d"]},
    ...
    def test_categorical_index_nonscalar_label(self):
        ...
        self.assertEqual(
            ...
            TabularMSA(
                [RNA("ACUA"), RNA("AAUG")],
                index=pd.CategoricalIndex(["b", "b"], categories=["a", "b", "c"]),
            ),
        )
    ...
    def test_float_index_out_of_order_slice(self):
        ...
        self.assertEqual(
            result,
            TabularMSA([DNA("ACGG"), DNA("AAGC"), DNA("ACTC")], index=[0.1, 2.4, 2.6]),
        )
    ...
    def test_nonscalar_fancy_index(self):
        ...
        with self.assertRaisesRegex(TypeError, "tuple.*independent.*MultiIndex"):
            self.get(msa, ["a", "b"])
...
class TestConstructor(unittest.TestCase):
    def setUp(self):
        ...
        self.msa = TabularMSA(
            self.seqs, metadata=self.m, positional_metadata=self.pm, index=self.index
        )
    ...
    def test_no_override(self):
        ...
        self.assertIsNot(result.positional_metadata, self.msa.positional_metadata)
...
class TestAppend(unittest.TestCase):
    ...
    def test_invalid_minter_index_reset_index_parameter_combos(self):
        ...
        for params in param_combos:
            with self.assertRaisesRegex(
                ValueError, "one of.*minter.*index.*reset_index"
            ):
    ...
    def test_minter_metadata_key(self):
        msa = TabularMSA(
            [DNA("", metadata={"id": "a"}), DNA("", metadata={"id": "b"})], minter="id"
        )
    ...
    def test_minter_callable(self):
        ...
        self.assertEqual(
            msa,
            TabularMSA(
                [DNA("", metadata={"id": "a"}), DNA("", metadata={"id": "b"}), DNA("")],
                index=["a", "b", ""],
            ),
        )
    ...
    def test_multiindex_minter_non_empty_msa(self):
        ...
        msa = TabularMSA([RNA("UU"), RNA("CA")], index=[("foo", 42), ("bar", 43)])

        msa.append(RNA("AC"), minter=multiindex_minter)

        self.assertIsInstance(msa.index, pd.MultiIndex)
        assert_index_equal(msa.index, pd.Index([("foo", 42), ("bar", 43), ("baz", 44)]))
    ...
    def test_index_non_empty_msa(self):
        ...
        self.assertEqual(
            msa, TabularMSA([DNA("AC"), DNA("GT"), DNA("--")], index=["a", "b", "foo"])
        )
    ...
    def test_multiindex_index_non_empty_msa(self):
        msa = TabularMSA([RNA("A"), RNA("C")], index=[("foo", 42), ("bar", 43)])
    ...
    def test_reset_index_default_index(self):
        ...
        self.assertEqual(msa, TabularMSA([DNA("ACGT"), DNA("CCCC"), DNA("ACGT")]))
        assert_index_equal(msa.index, pd.RangeIndex(3))
    ...
    def test_index_type_change(self):
        ...
        self.assertEqual(
            msa, TabularMSA([DNA("A"), DNA("."), DNA("C")], index=[0, 1, "foo"])
        )
    ...
    def test_duplicate_index(self):
        ...
        self.assertEqual(
            msa, TabularMSA([DNA("A"), DNA("."), DNA("C")], index=["foo", "bar", "foo"])
        )
    ...
    def test_empty_msa_with_positional_metadata_no_new_positions(self):
        ...
        self.assertEqual(msa, TabularMSA([DNA("")], positional_metadata={"foo": []}))
...
class TestExtend(unittest.TestCase):
    ...
    def test_from_tabular_msa_index_param_still_required(self):
        ...
        with self.assertRaisesRegex(ValueError, "one of.*minter.*index.*reset_index"):
    ...
    def test_invalid_minter(self):
        ...
        with self.assertRaises(KeyError):
            msa.extend([DNA("AAAA", metadata={"id": "foo"}), DNA("----")], minter="id")
    ...
    def test_sequences_index_length_mismatch(self):
        ...
        with self.assertRaisesRegex(ValueError, "sequences.*2.*index length.*3"):
    ...
    def test_minter_metadata_key(self):
        ...
        msa.extend(
            [DNA("", metadata={"id": "c"}), DNA("", metadata={"id": "d"})], minter="id"
        )
    ...
    def test_multiindex_minter_non_empty_msa(self):
        ...
        msa = TabularMSA([DNA("A"), DNA("G")], index=[("foo", 42), ("bar", 43)])

        msa.extend([DNA("C"), DNA("T")], minter=multiindex_minter)

        self.assertIsInstance(msa.index, pd.MultiIndex)
        assert_index_equal(
            msa.index, pd.Index([("foo", 42), ("bar", 43), ("baz", 44), ("baz", 45)])
        )
    ...
    def test_multiindex_index_non_empty_msa(self):
        msa = TabularMSA([DNA("."), DNA("-")], index=[("foo", 42), ("bar", 43)])
    ...
    def test_index_object_non_empty_msa(self):
        ...
        self.assertEqual(
            msa,
            TabularMSA(
                [DNA("CT"), DNA("GG"), DNA("AA"), DNA("GG")], index=[0, 1, 0, 1]
            ),
        )
    ...
    def test_reset_index_default_index(self):
        ...
        self.assertEqual(msa, TabularMSA([DNA("A"), DNA("G"), DNA("."), DNA("-")]))
    ...
    def test_reset_index_from_tabular_msa(self):
        ...
        msa.extend(
            TabularMSA([DNA("GG"), DNA("CC"), DNA("AA")], index=["a", "b", "c"]),
            reset_index=True,
        )

        self.assertEqual(
            msa, TabularMSA([DNA("AC"), DNA("TG"), DNA("GG"), DNA("CC"), DNA("AA")])
        )
    ...
    def test_empty_msa_with_positional_metadata_no_new_positions(self):
        ...
        self.assertEqual(
            msa, TabularMSA([DNA(""), DNA("")], positional_metadata={"foo": []})
        )
    ...
    def test_single_sequence(self):
        ...
        self.assertEqual(msa, TabularMSA([DNA("AC"), DNA("-C")], index=[0, "-C"]))
    ...
    def test_multiple_sequences(self):
        ...
        self.assertEqual(
            msa, TabularMSA([DNA("AC"), DNA("-C"), DNA("AG")], index=[0, "-C", "AG"])
        )
...
class TestJoin(unittest.TestCase):
    ...
    def test_duplicate_index_labels(self):
        with self.assertRaisesRegex(ValueError, "This MSA's index labels.*unique"):
            TabularMSA([DNA("AC"), DNA("--")], index=[0, 0]).join(
                TabularMSA([DNA("GT"), DNA("..")])
            )

        with self.assertRaisesRegex(ValueError, "`other`'s index labels.*unique"):
    ...
    def test_no_metadata(self):
        ...
        self.assertEqualJoinedMSA(joined, TabularMSA([DNA("AC-C"), DNA("G..G")]))
    ...
    def test_how_strict(self):
        ...
        self.assertEqualJoinedMSA(
            ...
            TabularMSA(
                ...
                positional_metadata={"bar": ["a", "b", "c", "d"], "foo": [1, 2, 3, 4]},
            ),
        )
    ...
    def test_how_strict_failure_positional_metadata_mismatch(self):
        ...
        msa2 = TabularMSA([DNA("-C"), DNA(".G")], positional_metadata={"foo": [3, 4]})

        with self.assertRaisesRegex(ValueError, "Positional metadata columns.*match"):
            msa1.join(msa2)
    ...
    def test_how_inner_no_index_overlap_with_positional_metadata_overlap(self):
        msa1 = TabularMSA([DNA("AC"), DNA("G.")], positional_metadata={"foo": [1, 2]})
        msa2 = TabularMSA(
            [DNA("-C"), DNA(".G")],
            index=["a", "b"],
    ...
    def test_how_outer(self):
        ...
        self.assertEqualJoinedMSA(
            joined,
            TabularMSA(
                [DNA("--..."), DNA("ACCAA"), DNA("G..GG"), DNA("C--CC"), DNA("-----")],
                index=range(-1, 4),
                positional_metadata={
...
class TestIterPositions(unittest.TestCase):
    ...
    def test_multiple_positions(self):
        ...
        self.assertEqual(
            obs, [Sequence("AA-"), Sequence("C--"), Sequence("GG-"), Sequence("T.-")]
        )
    ...
    def test_with_positional_metadata(self):
        ...
        seqs = [
            ...
            DNA("A-"),
            DNA("--", positional_metadata={"foo": [-1, -2], "bar": ["baz", "bazz"]}),
        ]
...
class TestConsensus(unittest.TestCase):
    ...
    def test_with_positional_metadata(self):
        ...
        self.assertEqual(
            cons,
            DNA(
                "A-T", positional_metadata={"foo": [42, 43, 42], "bar": ["a", "b", "c"]}
            ),
        )
...
class TestConservation(unittest.TestCase):
    ...
    def test_shannon_entropy_dna(self):
        ...
        actual = msa.conservation(metric="inverse_shannon_uncertainty")
        expected = np.array([1. - scipy.stats.entropy([0.5, 0.25, 0.25], base=4)])
        npt.assert_array_equal(actual, expected)
    ...
    def test_shannon_entropy_protein(self):
        ...
        msa = TabularMSA([Protein("A"), Protein("G"), Protein("C"), Protein("G")])
        actual = msa.conservation(metric="inverse_shannon_uncertainty")
        expected = np.array([1. - scipy.stats.entropy([0.5, 0.25, 0.25], base=20)])
        npt.assert_array_equal(actual, expected)
    ...
    def test_degenerate_mode_nan(self):
        ...
        expected = np.array([np.nan, np.nan, 1. - scipy.stats.entropy([1.0], base=4)])
        npt.assert_array_equal(actual, expected)
    ...
    def test_column_with_degen_and_gap(self):
        ...
        actual = msa.conservation(
            metric="inverse_shannon_uncertainty", degenerate_mode="nan", gap_mode="nan"
        )
    ...
    def test_gap_mode_nan(self):
        ...
        actual = msa.conservation(metric="inverse_shannon_uncertainty", gap_mode="nan")
        expected = np.array(
            [np.nan, np.nan, 1. - scipy.stats.entropy([1.0], base=4), np.nan]
        )
        npt.assert_array_equal(actual, expected)
...
class TestGetPosition(unittest.TestCase):
    ...
    def test_with_positional_metadata(self):
        msa = TabularMSA(
            [DNA("ACG"), DNA("A-G")],
            positional_metadata={"foo": [42, 43, 44], "bar": ["abc", "def", "ghi"]},
        )

        position = msa._get_position_(1)

        self.assertEqual(position, Sequence("C-", metadata={"foo": 43, "bar": "def"}))
...
class TestRepr(unittest.TestCase):
    def test_repr(self):
        ...
        self.assertEqual(repr(TabularMSA([DNA("")])), str(TabularMSA([DNA("")])))
        self.assertEqual(
