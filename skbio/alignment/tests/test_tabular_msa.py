...
class TestTabularMSA(unittest.TestCase, ReallyEqualMixin):
    ...
    def test_constructor_not_monomorphic(self):
        ...
        with self.assertRaisesRegex(TypeError, r"matching type.*float.*Protein"):
    ...
    def test_constructor_index_length_mismatch_iterable(self):
        with self.assertRaisesRegex(ValueError, r"sequences.*2.*index length.*0"):
    ...
    def test_reassign_index_mapping_invalid_type(self):
        ...
        with self.assertRaisesRegex(TypeError, r"mapping.*dict.*callable.*list"):
...
class TestLoc(SharedPropertyIndexTests, unittest.TestCase):
    ...
    def test_nonscalar_fancy_index(self):
        ...
        with self.assertRaisesRegex(TypeError, r"tuple.*independent.*MultiIndex"):
            self.get(msa, ["a", "b"])
...
class TestExtend(unittest.TestCase):
    ...
    def test_from_tabular_msa_index_param_still_required(self):
        ...
        with self.assertRaisesRegex(ValueError, r"one of.*minter.*index.*reset_index"):
    ...
    def test_sequences_index_length_mismatch(self):
        ...
        with self.assertRaisesRegex(ValueError, r"sequences.*2.*index length.*3"):
...
class TestJoin(unittest.TestCase):
    ...
    def test_duplicate_index_labels(self):
        with self.assertRaisesRegex(ValueError, r"This MSA's index labels.*unique"):
            TabularMSA([DNA("AC"), DNA("--")], index=[0, 0]).join(
                TabularMSA([DNA("GT"), DNA("..")])
            )

        with self.assertRaisesRegex(ValueError, r"`other`'s index labels.*unique"):
    ...
    def test_how_strict_failure_index_mismatch(self):
        ...
        with self.assertRaisesRegex(ValueError, r"Index labels must all " "match"):
            msa1.join(msa2)
    ...
    def test_how_strict_failure_positional_metadata_mismatch(self):
        ...
        with self.assertRaisesRegex(ValueError, r"Positional metadata columns.*match"):
            msa1.join(msa2)
...
class TestConservation(unittest.TestCase):
    ...
    def test_shannon_entropy_dna(self):
        ...
        actual = msa.conservation(metric="inverse_shannon_uncertainty")
        expected = np.array([1.0 - scipy.stats.entropy([0.5, 0.25, 0.25], base=4)])
        npt.assert_array_equal(actual, expected)
    ...
    def test_shannon_entropy_protein(self):
        ...
        actual = msa.conservation(metric="inverse_shannon_uncertainty")
        expected = np.array([1.0 - scipy.stats.entropy([0.5, 0.25, 0.25], base=22)])
        npt.assert_array_equal(actual, expected)
    ...
    def test_degenerate_mode_nan(self):
        ...
        expected = np.array([np.nan, np.nan, 1.0 - scipy.stats.entropy([1.0], base=4)])
        npt.assert_array_equal(actual, expected)
