...
class ReadableBinarySourceTests:
    ...
    def test_open_binary(self):
        self.check_open_state_contents(
            self.read_file, self.binary_contents, True, mode="r", encoding="binary"
        )
...
class ReadableSourceTest(unittest.TestCase):
    def setUp(self):
        ...
        self.gzip_encoded_file = self.get_fileobj(get_data_path("big5_file.gz"))
        self.bz2_encoded_file = self.get_fileobj(get_data_path("big5_file.bz2"))
...
class WritableBinarySourceTests:
    ...
    def check_open_state_contents(self, file, contents, is_binary, **kwargs):
        ...
        if is_binary:
            self.assertIsInstance(result, (io.BufferedWriter, io.BufferedRandom))
        else:
            self.assertIsInstance(result, io.TextIOBase)
    ...
    def test_open_binary(self):
        ...
        self.assertEqual(self.get_contents(self.binary_file), self.binary_contents)
    ...
    def test_open_encoding(self):
        ...
        self.assertEqual(self.get_contents(self.big5_file), self.encoded_contents)
    ...
    def test_open_gzip_encoding(self):
        ...
        self.compare_gzip_file_contents(
            self.get_contents(self.gzip_encoded_file), self.gzip_encoded_contents
        )
...
class TestReadNamedTemporaryFile(ReadableBinarySourceTests, ReadableSourceTest):
    expected_close = False
...
class TestWriteNamedTemporaryFile(WritableBinarySourceTests, WritableSourceTest):
    expected_close = False
