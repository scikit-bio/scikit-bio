...
class TestRegisterAndGetReader(RegistryTest):
    def test_get_reader_no_match(self):
        self.assertIs(None, self.registry.get_reader("not_a_format", MockClass))
    ...
    def test_register_reader_on_many(self):
        ...
        self.assertIs(format1_reader, self.registry.get_reader("format1", MockClassA))

        self.assertIs(format1_reader_b, self.registry.get_reader("format1", MockClassB))

        self.assertIs(format2_reader, self.registry.get_reader("format2", MockClassA))
        ...
        self.assertIs(format3_reader, self.registry.get_reader("format3", MockClassB))

        self.assertIs(format4_reader, self.registry.get_reader("format4", MockClassA))

        self.assertIs(format4_reader_b, self.registry.get_reader("format4", MockClassB))
    ...
    def test_register_reader_over_existing_override(self):
        ...
        self.assertIs(
            duplicate_format1_reader, self.registry.get_reader("format1", MockClassA)
        )
...
class TestRegisterAndGetWriter(RegistryTest):
    def test_get_writer_no_match(self):
        self.assertEqual(None, self.registry.get_writer("not_a_format", MockClass))
    ...
    def test_register_writer_on_many(self):
        ...
        self.assertIs(format4_writer, self.registry.get_writer("format4", MockClassA))

        self.assertIs(format4_writer_b, self.registry.get_writer("format4", MockClassB))
    ...
    def test_register_writer_over_existing_override(self):
        ...
        self.assertIs(format1_writer, self.registry.get_writer("format1", MockClassA))
        ...
        self.assertIs(
            duplicate_format1_writer, self.registry.get_writer("format1", MockClassA)
        )
...
class TestListReadFormats(RegistryTest):
    ...
    def test_one_read_format(self):
        ...
        self.assertEqual(["format1"], self.registry.list_read_formats(MockClass))
...
class TestListWriteFormats(RegistryTest):
    ...
    def test_one_write_format(self):
        ...
        self.assertEqual(["format1"], self.registry.list_write_formats(MockClass))
...
class TestRead(RegistryTest):
    ...
    def test_reader_exists_with_verify_true(self):
        ...
        instance = self.registry.read(fh, format="format1", into=MockClass, verify=True)
        self.assertEqual(MockClass([1, 2, 3, 4]), instance)
    ...
    def test_read_kwargs_passed_and_override(self):
        ...
        with warnings.catch_warnings(record=True):
            ...
            with self.assertRaises(ArgumentOverrideWarning):
                self.registry.read(
                    StringIO("notempty"), into=MockClass, arg3=[1], override=100
                )
    ...
    def test_file_sentinel_pass_none(self):
        ...
        instance = self.registry.read(fh, format="format1", into=MockClass, extra=None)
        self.assertEqual(MockClass([1, 2, 3, 4]), instance)
    ...
    def test_read_with_illegal_encoding(self):
        ...
        gen.close()
        # Should skip text sniffers
        instance = self.registry.read(self.fp1, encoding="binary", into=MockClass)
        ...
        with self.assertRaises(ValueError):
            self.registry.read(
                ["some content\n"], format="textf", encoding="binary", into=MockClass
            )
        ...
        with self.assertRaises(ValueError):
            self.registry.read(self.fp1, format="binf", encoding=None, into=MockClass)

        with self.assertRaises(ValueError):
            self.registry.read(
                self.fp1, format="binf", encoding=None, verify=False, into=MockClass
            )
...
class TestWrite(RegistryTest):
    ...
    def test_file_sentinel_many(self):
        ...
        self.registry.write(
            MockClass([]), format="format1", into=fh, extra=self.fp1, extra_2=self.fp2
        )
    ...
    def test_file_sentinel_pass_none(self):
        ...
        self.registry.write(MockClass([]), format="format1", into=fh, extra=None)
