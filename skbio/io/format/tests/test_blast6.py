...
class TestBlast6Reader(unittest.TestCase):
    ...
    def test_custom_valid_single_line(self):
        ...
        df = _blast6_to_data_frame(
            fp, columns=["qacc", "qseq", "btop", "sframe", "ppos", "positive", "gaps"]
        )
        exp = pd.DataFrame(
            [["query1", "PAAWWWWW", 8.0, 1.0, 100.00, 8.0, 0.0]],
            columns=["qacc", "qseq", "btop", "sframe", "ppos", "positive", "gaps"],
        )
        assert_data_frame_almost_equal(df, exp)
    ...
    def test_custom_and_default_passed_error(self):
        fp = get_data_path("blast6_default_single_line")
        with self.assertRaisesRegex(ValueError, r"`columns` and `default_columns`"):
    ...
    def test_wrong_column_name_error(self):
        fp = get_data_path("blast6_default_single_line")
        with self.assertRaisesRegex(ValueError, r"Unrecognized column.*'abcd'"):
