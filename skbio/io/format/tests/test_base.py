...
class PhredDecoderTests(unittest.TestCase):
    ...
    def test_empty_qual_str(self):
        npt.assert_equal(
            _decode_qual_to_phred("", variant="sanger"), np.array([], dtype=np.uint8)
        )
    ...
    def test_illumina13_variant(self):
        ...
        all_illumina13_ascii = (
            "@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijk" "lmnopqrstuvwxyz{|}~"
        )
    ...
    def test_illumina18_variant(self):
        ...
        all_illumina18_ascii = (
            "!\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKL" "MNOPQRSTUVWXYZ[\\]^_"
        )
...
class PhredEncoderTests(unittest.TestCase):
    ...
    def test_illumina13_variant(self):
        ...
        obs = npt.assert_warns(
            UserWarning, _encode_phred_to_qual, [42, 63, 33], variant="illumina1.3"
        )
    ...
    def test_illumina18_variant(self):
        ...
        obs = npt.assert_warns(
            UserWarning, _encode_phred_to_qual, [42, 63, 33], variant="illumina1.8"
        )
...
class TestFormatFASTALikeRecords(unittest.TestCase):
    def setUp(self):
        def generator():
            ...
            yield RNA("GAU", metadata={"id": "  foo \t\t bar ", "description": ""})
    ...
    def test_missing_quality_scores(self):
        ...
        with self.assertRaisesRegex(ValueError, r"2nd sequence.*quality scores"):
