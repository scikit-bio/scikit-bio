...
class SnifferTests(TestCase):
    def setUp(self):
        self.positive_fps = map(
            get_data_path, ["gff3_multi_record", "gff3_single_record", "gff3_dna"]
        )
        self.negative_fps = map(
            get_data_path, ["empty", "whitespace_only", "gff3_bad_missing_directive"]
        )
...
class ReaderTests(GFF3IOTests):
    ...
    def test_parse_record_raise(self):
        chars = "abc?!"
        for char in chars:
            lines = ["ctg123\t.\tgene\t1000\t9000\t.\t+\t%s\tID=gene00001" % char]
            with self.assertRaisesRegex(
                GFF3FormatError, "unknown value for phase column: '%s'" % char
            ):
    ...
    def test_gff3_to_interval_metadata_bad(self):
        with self.assertRaisesRegex(
            GFF3FormatError, "do not have 9 columns in this line"
        ):
            _gff3_to_interval_metadata(
    ...
    def test_gff3_to_generator(self):
        exps = [("Chromosome", self.imd1), ("gi|556503834|ref|NC_000913.3|", self.imd2)]
...
class WriterTests(GFF3IOTests):
    ...
    def test_interval_metadata_to_gff3_missing_field(self):
        ...
        imd.add([(0, 9)], metadata={"type": "gene", "ID": "gene00001", "Name": "EDEN"})
        with io.StringIO() as fh:
    ...
    def test_interval_metadata_to_gff3_multiple_values(self):
        ...
        imd = IntervalMetadata(9)
        imd.add(
            [(0, 9)], metadata={"type": "gene", "db_xref": ["GO:000152", "GO:001234"]}
        )
        with io.StringIO() as fh:
            _interval_metadata_to_gff3(imd, fh, seq_id="ctg123")
            # only compare the uncommented lines because the comments are not
...
class RoundtripTests(GFF3IOTests):
    def test_roundtrip_interval_metadata(self):
        ...
        with io.StringIO() as fh:
            _interval_metadata_to_gff3(
                _gff3_to_interval_metadata(self.single_fp, seq_id="Chromosome"),
                fh,
                seq_id="Chromosome",
            )
            obs = [i for i in fh.getvalue().splitlines() if not i.startswith("#")]
