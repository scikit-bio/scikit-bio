...
class TestStockholmReader(unittest.TestCase):
        ...
        exp = TabularMSA(
            [
                Protein(
                    ...
                    positional_metadata={
                        "SA": list(
                            "9998877564" "53524252.." "55152525.." "..36463774" "777"
                        )
                    },
                ),
                Protein(
                    ...
                    positional_metadata={
                        "SS": list(
                            "CCCCCCCHHHH" "HHHHHHH..HE" "EEEEEE....E" "EEEEEE" "EEEH"
                        )
                    },
                ),
                Protein(
                    ...
                    positional_metadata={
                        "AS": list(
                            "___________" "_____*_____" "___________" "________" "__"
                        ),
                        "IN": list(
                            "___________" "_1_________" "_____2_____" "_____0_" "___"
                        ),
                    },
                ),
            ],
            metadata={"ID": "CBS", "AC": "PF00571", "AU": "Bateman A", "SQ": "67"},
            positional_metadata={
                "SS_cons": list("CCCCCHHHHHHHH" "HHHHH..EEEEEE" "EE....EEEEEEE" "EEEH")
            },
    ...
    def test_stockholm_multiple_trees(self):
        ...
        msa = _stockholm_to_tabular_msa(fp, constructor=DNA)
        exp = TabularMSA(
            [], metadata={"NH": {"tree1": "ABCD", "tree2": "EFGH", "tree3": "IJKL"}}
        )
        self.assertEqual(msa, exp)
    ...
    def test_stockholm_to_msa_different_padding(self):
        ...
        exp = TabularMSA(
            ...
            metadata={
                "RN": [
                    OrderedDict([("RC", "A Runon Comment Without " "Whitespace")]),
                    OrderedDict([("RC", "A Runon Comment With " "Whitespace")]),
                ]
            },
        )
        self.assertEqual(msa, exp)
    ...
    def test_stockholm_handles_missing_reference_items(self):
        ...
        exp = TabularMSA(
            [],
            metadata={"RN": [OrderedDict([("RT", "A Title"), ("RA", "The Author")])]},
        )
        self.assertEqual(msa, exp)
    ...
    def test_stockholm_duplicate_tree_id_error(self):
        ...
        with self.assertRaisesRegex(StockholmFormatError, "Tree.*tree1.*in file."):
    ...
    def test_stockholm_missing_reference_number_error(self):
        ...
        with self.assertRaisesRegex(StockholmFormatError, "Expected 'RN'.*'RL' tag."):
            _stockholm_to_tabular_msa(fp, constructor=DNA)

    def test_nonexistent_gr_error(self):
        fp = get_data_path("stockholm_invalid_nonexistent_gr")
        with self.assertRaisesRegex(
            StockholmFormatError, "GS or GR.*nonexistent sequence.*RL1355."
        ):
    ...
    def test_nonexistent_gs_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError, "GS or GR.*nonexistent sequence.*AC14."
        ):
            _stockholm_to_tabular_msa(fp, constructor=RNA)
    ...
    def test_duplicate_gr_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError,
            "Found duplicate GR.*OS.*LFDR3.*supported " "by the reader.",
        ):
            _stockholm_to_tabular_msa(fp, constructor=DNA)
    ...
    def test_duplicate_gc_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError,
            "Found duplicate GC.*SS_cons.*supported " "by the reader.",
        ):
            _stockholm_to_tabular_msa(fp, constructor=DNA)
    ...
    def test_missing_header_error(self):
        ...
        with self.assertRaisesRegex(StockholmFormatError, "File missing.*header"):
            _stockholm_to_tabular_msa(fp, constructor=DNA)

    def test_missing_footer_error(self):
        fp = get_data_path("stockholm_missing_footer")
        with self.assertRaisesRegex(StockholmFormatError, 'Final line.*only "//".'):
            _stockholm_to_tabular_msa(fp, constructor=DNA)

    def test_data_type_error(self):
        fp = get_data_path("stockholm_invalid_data_type")
        with self.assertRaisesRegex(StockholmFormatError, "Unrecognized.*'#=GZ"):
            _stockholm_to_tabular_msa(fp, constructor=DNA)

    def test_malformed_gf_line_error(self):
        fp = get_data_path("stockholm_malformed_gf_line")
        with self.assertRaisesRegex(
            StockholmFormatError, "Line contains 2.*must contain.*3."
        ):
            _stockholm_to_tabular_msa(fp, constructor=DNA)
    ...
    def test_malformed_gs_line_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError, "Line contains 3.*must contain.*4."
        ):
            _stockholm_to_tabular_msa(fp, constructor=DNA)
    ...
    def test_malformed_gr_line_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError, "Line contains 2.*must contain.*4."
        ):
            _stockholm_to_tabular_msa(fp, constructor=DNA)
    ...
    def test_malformed_data_line_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError, "Line contains 1.*must contain.*2."
        ):
            _stockholm_to_tabular_msa(fp, constructor=DNA)
    ...
    def test_unsupported_constructor_error(self):
        fp = get_data_path("empty")
        with self.assertRaisesRegex(TypeError, "`constructor`.*`GrammaredSequence`."):
...
class TestStockholmWriter(unittest.TestCase):
            ...
            metadata=OrderedDict(
                [
                    (
                        "NH",
                        OrderedDict(
                            [("tree1", "ABCD"), ("tree2", "EFGH"), ("tree3", "IJKL")]
                        ),
                    )
                ]
            ),
    ...
    def test_msa_to_stockholm_data_only(self):
        ...
        msa = TabularMSA(
            [RNA("ACUCCGACAUGCUCC"), RNA("UAGUGCCGAACGCUG"), RNA("GUGUGGGCGUGAUUC")],
            index=["seq1", "seq2", "seq3"],
        )
        fh = io.StringIO()
        _tabular_msa_to_stockholm(msa, fh)
    ...
    def test_unoriginal_index_error(self):
        msa = TabularMSA([DNA("ATCGCCAGCT"), DNA("TTGTGCTGGC")], index=["seq1", "seq1"])
        with self.assertRaisesRegex(
            StockholmFormatError, "index labels must be unique."
        ):
            fh = io.StringIO()
            _tabular_msa_to_stockholm(msa, fh)

    def test_unoriginal_gr_feature_names_error(self):
        pos_metadata_dataframe = pd.DataFrame.from_items(
            [
                ("AC", list("GAGCAAGCCACTA" "GA")),
                ("SS", list("TCCTTGAACTACC" "CG")),
                ("AS", list("TCAGCTCTGCAGC" "GT")),
                ("SS", list("GTCAGGCGCTCGG" "TG")),
            ]
        )
        msa = TabularMSA(
            [DNA("CGTCAATCTCGAACT", positional_metadata=pos_metadata_dataframe)],
            index=["seq1"],
        )
        with self.assertRaisesRegex(
            StockholmFormatError,
    ...
    def test_unoriginal_gc_feature_names_error(self):
        pos_metadata_dataframe = pd.DataFrame.from_items(
            [
                ("AC", list("GAGCAAGCCACTA" "GA")),
                ("SS", list("TCCTTGAACTACC" "CG")),
                ("SS", list("TCAGCTCTGCAGC" "GT")),
                ("AC", list("GTCAGGCGCTCGG" "TG")),
            ]
        )
        msa = TabularMSA(
            [DNA("CCCCTGCTTTCGTAG")], positional_metadata=pos_metadata_dataframe
        )
        with self.assertRaisesRegex(
            StockholmFormatError,
    ...
    def test_gr_wrong_dataframe_item_length_error(self):
        ...
        pos_metadata_dataframe = pd.DataFrame(
            {"AC": seq1, "SS": list("TCCTTGAACTACCCGA"), "AS": list("TCAGCTCTGCAGCGTT")}
        )
        msa = TabularMSA(
            [DNA("TCCTTGAACTACCCGA", positional_metadata=pos_metadata_dataframe)]
        )
        with self.assertRaisesRegex(
            StockholmFormatError,
            "Sequence-specific positional metadata.*"
    ...
    def test_rn_not_list_of_refs_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError, "Expected 'RN'.*list of reference" ".*got '1'"
        ):
            fh = io.StringIO()
            _tabular_msa_to_stockholm(msa, fh)
    ...
    def test_rn_data_not_in_dict_error(self):
        msa = TabularMSA(
            [], metadata={"RN": [OrderedDict([("RL", "Flagstaff")]), "Incorrect Item"]}
        )
        with self.assertRaisesRegex(
            StockholmFormatError,
    ...
    def test_invalid_reference_tag_error(self):
        msa = TabularMSA(
            [], metadata={"RN": [OrderedDict([("RL", "Flagstaff"), ("foo", "bar")])]}
        )
        with self.assertRaisesRegex(
            StockholmFormatError,
            "Invalid reference.*foo' found in.*1.*Vali" "d reference tags are:",
        ):
            fh = io.StringIO()
            _tabular_msa_to_stockholm(msa, fh)
