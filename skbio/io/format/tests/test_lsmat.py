...
class DissimilarityAndDistanceMatrixReaderWriterTests(LSMatTestData):
    def setUp(self):
        ...
        self.lsmat_3x3_data = [[0.0, 0.01, 4.2], [0.01, 0.0, 12.0], [4.2, 12.0, 0.0]]
        ...
        self.dissim_strs = [LSMat_1x1, LSMat_2x2, LSMat_2x2_ASYM, LSMat_3x3, LSMat_3x3]
    ...
    def test_read_valid_files(self):
        for fn, cls, objs, fhs in (
            ...
            (_lsmat_to_distance_matrix, DistanceMatrix, self.dist_objs, self.dist_fhs),
        ):
    ...
    def test_read_invalid_files(self):
        for fn in _lsmat_to_dissimilarity_matrix, _lsmat_to_distance_matrix:
            for invalid_fh, error_msg_regexp in self.invalid_fhs:
                with self.assertRaisesRegex(LSMatFormatError, error_msg_regexp):
                    invalid_fh.seek(0)
                    fn(invalid_fh)
...
class SnifferTests(LSMatTestData):
    ...
    def test_match_tsv(self):
        ...
        fhs = self.valid_fhs + [self.invalid_1_fh, self.invalid_3_fh, self.invalid_4_fh]
        for fh in fhs:
