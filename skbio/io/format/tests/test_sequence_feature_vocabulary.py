...
class Tests(TestCase):
    def test_parse_section_default(self):
        lines = [["FOO  blah blah", "     blah"], ["FOO=blah", "    blah"], ["FOO"]]
        kwargs = [
            {"join_delimiter": "=", "return_label": False},
            {"label_delimiter": "=", "join_delimiter": "", "return_label": True},
            {"label_delimiter": "=", "join_delimiter": "=", "return_label": True},
        ]
    ...
    def test_parse_loc_str(self):
        ...
        expects = [
            ...
            ([(2, 8)], [(False, True)], {"strand": "-"}),
            ([(2, 5), (6, 9)], [(False, True), (True, False)], {"strand": "-"}),
            ([(2, 8)], [(False, False)], {"strand": "+"}),
            ([(2, 8)], [(False, False)], {"strand": "+"}),
    ...
    def test_parse_loc_str_invalid(self):
        ...
        for example in examples:
            with self.assertRaisesRegex(
                FileFormatError, r"Could not parse location string: " '"%s"' % example
            ):
    ...
    def test_serialize_location(self):
        ...
        i6 = imd.add(
            [(0, 2), (3, 9)], [(True, False), (False, True)], metadata={"strand": "-"}
        )
