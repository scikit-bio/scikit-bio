    ...
    def setUp(self):
        ...
        self.invalid_fps = map(
            ...
            [
                ...
                ("ordination_error2", r"Proportion explained header not found", False),
                ...
                ("ordination_error18", r"proportion explained.*eigvals: 8 != 9", True),
                ("ordination_error19", r"coordinates.*species.*eigvals: 1 != 2", True),
                ("ordination_error20", r"coordinates.*site.*eigvals: 1 != 2", True),
                ...
                ("ordination_error24", r"end of file.*row 2.*Species section", True),
            ],
        )
...
class OrdinationResultsReaderWriterTests(OrdinationTestData):
    def setUp(self):
        ...
        site_constraints = pd.DataFrame(
            np.loadtxt(get_data_path("ordination_exp_Ordination_CCA_site_constraints")),
            index=site_ids,
            columns=axes_ids,
        )
        prop_explained = None
        ...
        site_constraints = pd.DataFrame(
            np.loadtxt(get_data_path("ordination_exp_Ordination_RDA_site_constraints")),
            index=site_ids,
            columns=axes_ids,
        )
        prop_explained = None
        ...
        self.ordination_results_objs = [ca_scores, cca_scores, pcoa_scores, rda_scores]
    ...
    def test_read_invalid_files(self):
        for invalid_fp, error_msg_regexp, _ in self.invalid_fps:
            with self.assertRaisesRegex(OrdinationFormatError, error_msg_regexp):
...
class SnifferTests(OrdinationTestData):
    ...
    def test_matches_and_nonmatches(self):
        ...
        for fp, _, expected_sniffer_match in self.invalid_fps:
            self.assertEqual(_ordination_sniffer(fp), (expected_sniffer_match, {}))
