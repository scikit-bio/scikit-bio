...
class TestReaders(unittest.TestCase):
    def setUp(self):
        ...
        self.invalid_files = [
            ...
            for e in [
                ("empty", PhylipFormatError, "This file is empty."),
                ("whitespace_only", PhylipFormatError, 'Found non-header line .*: ""'),
                (
                    "phylip_invalid_empty_line_after_header",
                    PhylipFormatError,
...
class TestWriters(unittest.TestCase):
    def setUp(self):
        ...
        single_seq_long = TabularMSA(
            [DNA("..ACC-GTTGG..AATGC.C----", metadata={"id": "foo"})], minter="id"
        )

        # single sequence with only a single character (minimal writeable
        # alignment)
        single_seq_short = TabularMSA([DNA("-", metadata={"id": ""})], minter="id")
        ...
        self.invalid_objs = [
            ...
            (
                TabularMSA(
                    [DNA("", metadata={"id": "d1"}), DNA("", metadata={"id": "d2"})]
                ),
                "one position",
            ),
            # ids too long
