...
class ReaderTests(TestCase):
    def setUp(self):
        ...
        self.single = (
            ...
            {},
            list(map(get_data_path, ["fasta_single_seq", "fasta_max_width_1"])),
        ...
        self.multi = (
            [
                ...
                Sequence(
                    ...
                    positional_metadata={"quality": np.asarray([42], dtype=np.uint8)},
                ),
                Sequence(
                    "AACGGuA",
                    metadata={"id": "", "description": "desc3"},
                    positional_metadata={
                        "quality": np.asarray([0, 0, 0, 0, 0, 0, 0], dtype=np.uint8)
                    },
                ),
                Sequence(
                    "ACGTTGCAccGG",
                    ...
                    positional_metadata={
                        "quality": np.asarray(
                            [55, 10, 0, 99, 1, 1, 8, 77, 40, 10, 10, 0], dtype=np.uint8
                        )
                    },
                ),
                Sequence(
                ...
                Sequence(
                    ...
                    positional_metadata={
                        "quality": np.asarray(
                            [42, 42, 255, 255, 42, 42, 42, 42, 42, 43], dtype=np.uint8
                        )
                    },
                ),
            ],
        ...
        self.odd_labels_different_type = (
            [
                Protein(
                    ...
                    metadata={"id": "", "description": ""},
                    positional_metadata={
                        "quality": np.asarray([0, 0, 1, 5, 44, 0], dtype=np.uint8)
                    },
                    validate=False,
                ),
                Protein(
        ...
        for exp, kwargs, fasta_fps, qual_fps in test_cases:
            for fasta_fp in fasta_fps:
                ...
                for qual_fp in qual_fps:
                    obs = list(_fasta_to_generator(fasta_fp, qual=qual_fp, **kwargs))
    ...
    def test_fasta_to_any_sequence(self):
        for constructor, reader_fn in (
            ...
            (
                ...
                partial(_fasta_to_protein, validate=False, lowercase="introns"),
            ),
        ):
            ...
            for fasta_fp in fasta_fps:
                ...
                exp.positional_metadata.insert(
                    0,
                    "quality",
                    np.asarray([10, 20, 30, 10, 0, 0, 0, 255, 1, 255], dtype=np.uint8),
                )
                qual_fps = list(
            ...
            qual_fps = list(map(get_data_path, ["qual_multi_seq", "qual_max_width_5"]))
            for fasta_fp in fasta_fps:
                # get first
                exp = constructor(
                    "ACGT-acgt.", metadata={"id": "seq1", "description": "desc1"}
    ...
    def test_fasta_to_tabular_msa(self):
        ...
        for exp_list, kwargs, fasta_fps, qual_fps in test_cases:
            ...
            for fasta_fp in fasta_fps:
                ...
                for qual_fp in qual_fps:
                    obs = _fasta_to_tabular_msa(fasta_fp, qual=qual_fp, **kwargs)
                    self.assertEqual(obs, exp)
...
class WriterTests(TestCase):
    def setUp(self):
        self.bio_seq1 = DNA(
            "ACGT-acgt.",
            metadata={"id": "seq1", "description": "desc1"},
            positional_metadata={"quality": [10, 20, 30, 10, 0, 0, 0, 255, 1, 255]},
            lowercase="introns",
        )
        self.bio_seq2 = DNA(
            "A",
        ...
        self.prot_seq = Protein(
            ...
            positional_metadata={"quality": [42, 42, 255, 255, 42, 42, 42, 42, 42, 43]},
            lowercase="introns",
        )
        ...
        seqs = [
            ...
            CustomSequence(
                ...
                positional_metadata={"quality": [12345, 678909, 999999, 4242424242]},
            ),
        ]
        ...
        self.objs_fps = list(
            map(
                lambda e: (e[0], e[1], get_data_path(e[2]), get_data_path(e[3])),
                [
                    ...
                    (
                        newline_description_gen(),
                        {"description_newline_replacement": "", "lowercase": "introns"},
                        "fasta_description_newline_replacement_empty_str",
                        "qual_description_newline_replacement_empty_str",
                    ),
                    (
        ...
        self.invalid_objs = [
            ...
            (single_seq_gen(), {"max_width": 0}, ValueError, "max_width=0"),
            (
                multi_seq_gen(),
                {"id_whitespace_replacement": "-\n_"},
                ValueError,
...
class RoundtripTests(TestCase):
    def test_roundtrip_generators(self):
        ...
        for fasta_fp, qual_fp in fps:
            ...
            qual_fh = io.StringIO()
            _generator_to_fasta(
                _fasta_to_generator(fasta_fp, qual=qual_fp), fasta_fh, qual=qual_fh
            )
            obs_fasta = fasta_fh.getvalue()
            obs_qual = qual_fh.getvalue()
            fasta_fh.close()
