...
def urlopen_patch(response_code=200, parse_mode="text", multiple_seqs=False):
    """Interjects the urlopen function to return a mocked (but parseble) HTTPResponse."""
...
class TestOffline(unittest.TestCase):
    ...
    def test_parsing(self):
        """Tests that `entrez._parse_response` can correctly interpret an empty response in various
        data formats."""
        # The call to the entrez function doesn't need to conform to an actually valid call since
        ...
        with urlopen_patch(parse_mode="fasta", multiple_seqs=False):
            single_fasta_result = entrez.efetch(db="", id="", rettype="fasta", parse=True)
        with urlopen_patch(parse_mode="fasta", multiple_seqs=True):
            multiple_fasta_result = entrez.efetch(db="", id="", rettype="fasta", parse=True)
        with urlopen_patch(parse_mode="gb", multiple_seqs=False):
            single_gb_result = entrez.efetch(db="", id="", rettype="gb", parse=True)
        with urlopen_patch(parse_mode="gb", multiple_seqs=True):
            multiple_gb_result = entrez.efetch(db="", id="", rettype="gb", parse=True)
    ...
    def test_einfo(self):
        ...
        expected_url_2 = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/einfo.fcgi?retmode=json"
        with urlopen_patch() as patch:
            entrez.einfo(retmode="json")
        self.assertEqual(expected_url_2, get_url_from_patch(patch))

        expected_url_3 = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/einfo.fcgi?" \
    ...
    def test_esearch(self):
        expected_url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&" \
                       "term=cancer&usehistory=y&retstart=0&retmax=100&rettype=uilist&" \
                       "retmode=xml&datetype=edat&reldate=60"
        with urlopen_patch() as patch:
            entrez.esearch("pubmed", "cancer", reldate=60, datetype="edat", retmax=100,
                           usehistory='y')
        self.assertEqual(expected_url, get_url_from_patch(patch))

        expected_url_2 = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?" \
                         "db=nucleotide&term=biomol+trna%5Bprop%5D&retstart=0&retmax=20" \
                         "&rettype=uilist&retmode=xml"
        with urlopen_patch() as patch:
            entrez.esearch("nucleotide", "biomol trna[prop]")
        self.assertEqual(expected_url_2, get_url_from_patch(patch))

        expected_url_3 = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=protein" \
                         "&term=70000%3A90000%5Bmolecular+weight%5D&retstart=0&retmax=20" \
                         "&rettype=uilist&retmode=xml"
        with urlopen_patch() as patch:
    ...
    def test_epost(self):
        expected_url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/epost.fcgi?db=pubmed" \
    ...
    def test_esummary(self):
        expected_url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed" \
                       "&id=11850928%2C11482001&retstart=0&retmode=xml&version=2.0"
        with urlopen_patch() as patch:
            entrez.esummary("pubmed", id=[11850928, 11482001], version=2)
        self.assertEqual(expected_url, get_url_from_patch(patch))

        expected_url_2 = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?" \
                         "db=nucleotide&id=28864546%2C28800981&retstart=0&retmode=xml"
        with urlopen_patch() as patch:
            entrez.esummary("nucleotide", id=[28864546, 28800981])
        self.assertEqual(expected_url_2, get_url_from_patch(patch))

    def test_efetch(self):
        expected_url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?" \
                       "db=pubmed&id=11748933%2C11700088&retmode=xml&retstart=0"
        with urlopen_patch() as patch:
            entrez.efetch("pubmed", id=[11748933, 11700088], retmode="xml")
        self.assertEqual(expected_url, get_url_from_patch(patch))

        expected_url_2 = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?" \
                         "db=nuccore&id=21614549&retmode=text&rettype=fasta&retstart=0" \
                         "&strand=1&seq_start=1&seq_stop=100"
        with urlopen_patch() as patch:
            entrez.efetch("nuccore", id=21614549, strand=1, seq_start=1, seq_stop=100,
                          rettype="fasta", retmode="text")
        self.assertEqual(expected_url_2, get_url_from_patch(patch))

    def test_elink(self):
        expected_url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/elink.fcgi?db=gene&" \
                       "dbfrom=protein&cmd=neighbor&id=15718680%2C157427902&retmode=xml"
        with urlopen_patch() as patch:
            entrez.elink("gene", "protein", id=[15718680, 157427902])
        self.assertEqual(expected_url, get_url_from_patch(patch))

        expected_url_2 = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/elink.fcgi?" \
    ...
    def test_egquery(self):
        expected_url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/egquery?term=asthma" \
                       "&retmode=xml"
        with urlopen_patch() as patch:
    ...
    def test_espell(self):
        expected_url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/espell.fcgi?" \
    ...
    def test_ecitmatch(self):
        expected_url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/ecitmatch.cgi?" \
                       "db=pubmed&retmode=xml&bdata=proc+natl+acad+sci+u+s+a%7C1991%" \
                       "7C88%7C3248%7Cmann+bj%7CArt1%7C%0Dscience%7C1987%7C235%7C18" \
                       "2%7Cpalmenberg+ac%7CArt2%7C"
        with urlopen_patch() as patch:
            entrez.ecitmatch("proc natl acad sci u s a|1991|88|3248|mann bj|Art1|\rscience|1987"
...
class TestOnline(unittest.TestCase):
    ...
    def test_einfo(self):
        ...
        self.assertElementsEqual(expected_result.find(".//DbList"), result.find(".//DbList"))

        expected_result_2 = ET.parse(get_data_path("einfo2.xml")).getroot()
        result_2 = entrez.einfo("protein", version=2, parse=True)
        self.assertElementsEqual(expected_result_2.find(".//DbInfo"), result_2.find(".//DbInfo"))

    def test_esearch(self):
        expected_result = ET.parse(get_data_path("esearch.xml")).getroot()
        result = entrez.esearch("pubmed", "cancer", reldate=60, datetype="edat", retmax=100,
                                usehistory='y', parse=True)
        self.assertElementsEqual(expected_result.find(".//IdList"), result.find(".//IdList"))

        expected_result_2 = ET.parse(get_data_path("esearch2.xml")).getroot()
        result_2 = entrez.esearch("nucleotide", "biomol trna[prop]", parse=True)
        self.assertElementsEqual(expected_result_2.find(".//IdList"), result_2.find(".//IdList"))

        expected_result_3 = ET.parse(get_data_path("esearch3.xml")).getroot()
        result_3 = entrez.esearch("protein", "70000:90000[molecular weight]", parse=True)
        self.assertElementsEqual(expected_result_3.find(".//IdList"), result_3.find(".//IdList"))

    def test_epost(self):
        expected_result = ET.parse(get_data_path("epost.xml")).getroot()
        result = entrez.epost("pubmed", id=[11237011, 12466850], parse=True)
        self.assertElementsEqual(expected_result.find(".//QueryKey"), result.find(".//QueryKey"))

    def test_esummary(self):
        expected_result = ET.parse(get_data_path("esummary.xml")).getroot()
        result = entrez.esummary("pubmed", id=[11850928, 11482001], version=2, parse=True)
        for expected, actual in zip(expected_result.findall(".//DocumentSummary"),
        ...
        result_2 = entrez.esummary("nucleotide", id=[28864546, 28800981], parse=True)
    ...
    def test_efetch(self):
        ...
        result = entrez.efetch("pubmed", id=[11748933, 11700088], retmode="xml", parse=True)
        for expected, actual in zip(expected_result.findall(".//PMID"), result.findall(".//PMID")):
            self.assertElementsEqual(expected, actual)

        expected_result_2 = DNA.read(get_data_path("efetch2.fasta"))
        result_2 = entrez.efetch("nuccore", id=21614549, strand=1, seq_start=1, seq_stop=100,
    ...
    def test_elink(self):
        ...
        result = entrez.elink("gene", "protein", id=[15718680, 157427902], parse=True)
    ...
    def test_ecitmatch(self):
        ...
        result = entrez.ecitmatch("proc natl acad sci u s a|1991|88|3248|mann bj|Art1|\rscience"
