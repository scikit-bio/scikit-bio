...
class TestInterval(unittest.TestCase, ReallyEqualMixin):
    ...
    def test_repr(self):
        f = Interval(
            interval_metadata=self.im, bounds=[(1, 2)], metadata={"name": "sagA"}
        )
        exp = (
    ...
    def test_set_metadata(self):
        ...
        self.assertDictEqual(f.metadata, {"name": "sagB", "function": "transport"})
        f.metadata = {}
        self.assertDictEqual(f.metadata, {})
...
class TestIntervalMetadata(unittest.TestCase, ReallyEqualMixin):
    ...
    def test_deepcopy(self):
        ...
        self.assertEqual(i2.metadata, {"gene": "sagB", "bound": 0, "spam": [0, 1]})
        self.assertEqual(i1.metadata, {"gene": "sagB", "bound": 0, "spam": [0]})
    ...
    def test_init_copy_from_error(self):
        ...
        with self.assertRaisesRegex(ValueError, r"larger than upper bound \(%r\)" % i):
    ...
    def test_concat(self):
        ...
        exp.add(bounds=[(3, 6)], fuzzy=[(True, False)], metadata={"gene": "sagA"})
        exp.add(bounds=[(5, 7)], metadata={"gene": "sagB"})
        exp.add(bounds=[(8, 12)], fuzzy=[(False, True)], metadata={"gene": "sagC"})
        self.assertEqual(obs, exp)
    ...
    def test_merge_unbounded_to_bounded(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            r"Cannot merge an unbound IntervalMetadata object " "to a bounded one",
        ):
            self.im_1.merge(im)
    ...
    def test_sort(self):
        ...
        self.assertEqual(self.im_2._intervals, [self.im_2_2, self.im_2_1, interval])

        self.im_2.sort()
        self.assertEqual(im, self.im_2)
        self.assertEqual(self.im_2._intervals, [interval, self.im_2_1, self.im_2_2])
    ...
    def test_add_eq_start_end_bound(self):
        for i in 0, 1, self.upper_bound:
            # test that no exception is raised
            self.im_empty.add(bounds=[(i, i)], metadata={"gene": "sagA", "bound": 0})
    ...
    def test_query(self):
        intervals = list(self.im_2.query(bounds=[(1, 5)], metadata={"gene": "sagA"}))
        self.assertEqual(len(intervals), 1)
        self.assertEqual(intervals[0], self.im_2_1)
    ...
    def test_query_no_hits(self):
        ...
        intervals = list(self.im_2.query(bounds=[(1, 2)], metadata={"gene": "sagC"}))
        self.assertEqual(len(intervals), 0)
    ...
    def test_eq_ne(self):
        ...
        im1.add(metadata={"gene": "sagA", "bound": "0"}, bounds=[(0, 2), (4, 7)])
        ...
        im2.add(metadata={"gene": "sagA", "bound": "0"}, bounds=[(0, 2), (4, 7)])

        im3 = IntervalMetadata(10)
        im3.add(metadata={"gene": "sagA", "bound": "3"}, bounds=[(0, 2), (4, 7)])
