...
from skbio.util._decorator import stable, experimental, deprecated, _state_decorator
...
class TestDeprecated(TestStabilityState):
    ...
    def test_function_output(self):
        f = self._get_f("0.1.0", until="0.1.4", reason="You should now use skbio.g().")
        self.assertEqual(f(1), 43)
    ...
    def test_function_docstring(self):
        ...
        f = self._get_f("0.1.1", until="0.1.5", reason="You should now use skbio.h().")
        e1 = (
            " Add 42, or something else, to x.\n\n"
